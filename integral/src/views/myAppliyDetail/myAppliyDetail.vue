    ...
    <div class="myAppliyDetail-option">
      ...
      <table border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td>
            {{applyDetaData.agentProvince}}
            {{applyDetaData.agentCity}}
            {{applyDetaData.agentZone}}
          </td>
        </tr>
      </table>
    </div>
    ...
    <div class="myAppliyDetail-option">
      ...
      <table border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td>
            <span v-if="applyDetaData.payStatus === 0 && applyDetaData.paymentVoucher !== ''">支付审核中</span>
            <span v-else>{{applyDetaData.payStatus | paySteFilter}}</span>
            <span @click.stop="showReasonPop = true" class="reason"  v-if="applyDetaData.payAuditStatus == 3">原因</span>
          </td>
        </tr>
      </table>
    </div>
    <div class="myAppliyDetail-option" style="border-bottom:none;">
      <div class="myAppliyDetail-option-title">
        <div>开通信息</div>
        <div></div>
      </div>
      <table border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td>{{applyDetaData.isOpen | isActiveFilter}}</td>
        </tr>
        <tr>
          <td align="right">原因</td>
          <td v-if="applyDetaData.payStatus === 2">{{applyDetaData.payAuditInfo}}</td>
          <td v-else>{{applyDetaData.openInfo}}</td>
        </tr>
      </table>
    </div>
    <!--失败原因-->
