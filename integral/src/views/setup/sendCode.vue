<template>
  <div class="verifyingPhone">
    ...
    <div class="verifyingPhone-phone">{{userInfo.phone | filterPhone}}</div>
    <div class="sub-btn-wrap" style="padding: 0 .3rem;">
      <div class="ta-c sub-btn active" @click="sendCode(1)">发送验证码</div>
    </div>
    ...
    <div v-if="showCodePop">
      <x-dialog  v-model="showCodePop"  :hide-on-blur="true" :dialog-style="{'text-align': 'left', width: '80%', 'max-width': '7.5rem'}">
        <div class="CodePop">
          ...
          <div class="CodePop-phone">{{userInfo.phone | filterPhone}}</div>
          <div class="CodePop-tip">请查看短信并输入验证码</div>
          <input-password v-model="code" :length="4" inputType="code" @success="confirmEvent"></input-password>
    ...
    methods: {
      ...
      */
      confirmEvent () {
        this.checkCode(() => {
          this.closeCodePop()
          this.$router.push({path: '/setup/setPayPassword'})
