  ...
  <common-tpl class="assemble-wrap">
    <template slot="header" class="search-wrap">
      ...
      <high-search @search="searchHandle" :textVisible = "false">
        ...
        <template slot="edit">
          <!-- <el-button :disabled="!tableData.length" @click="$router.push({path: '/admin/operation/extension/assemble/add', query: {type: 5}})">添加拼团活动</el-button> -->
           <el-button @click="addAssembleHandle">添加拼团活动</el-button>
        </template>
      </high-search>
    </template>
    ...
    <template slot="main">
      <el-table :data="tableData" style="width: 100%" v-loading="loading" element-loading-text="加载中">
        <el-table-column label="扫码预览">
          <template slot-scope="scope">
            <!-- 缩略图 -->
            <thumbnail-component :list="[{url: scope.row.qrcodeImage}]" :size="40"></thumbnail-component>
          </template>
        </el-table-column>
        ...
        <el-table-column label="参与人数">
          <template slot-scope="scope">{{scope.row.joinNum | filterEmpty}}</template>
        </el-table-column>

        <el-table-column label="访问人数">
          <template slot-scope="scope">{{scope.row.visitNum | filterEmpty}}</template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getListData () {
      this.loading = true
      this.$http.get('@ROOT_API/group/getGroupPage', {
        params: {
    ...
    addAssembleHandle () {
      this.$http.post('@ROOT_API/shareTheme/getIdByInit', {
        ...
        if (results) {
          this.$router.push({path: '/admin/operation/extension/assemble/add', query: {themeId: results, type: 5}})
        }
      })
    },
    /**
    ...
    handleEdit (row) {
      this.$router.push({
        ...
        query: {id: row.id, type: 5, status: row.status}
      })
    },
    /**
     * 搜索
     ...
     */
    deleteHandle () {
      this.$http.post('@ROOT_API/group/delGroup', {
