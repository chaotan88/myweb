      ...
      <el-form :model="formData" ref="formData" label-width="120px" v-loading="loading">
              ...
              <div class="partition-list-box">
                <div class="item-list" :style="{width: groupList.length * 120 + 'px', 'margin-left': -columnPage * 120 * 5 + 'px'}">
                  <div class="pos-r item" v-for="(item, index) in groupList" >
                    ...
                    <div class="pos-a btns-wrap ta-c" @click.stop.prevent=""  :class="{select: formData.goodsGroupId === item.groupId && item.groupSort !== 0}">
        ...
        <gray-title title="商品设置">
          <template v-if="formData.goodsGroupSort === 0">
            <el-button slot="right" size="small" type="primary" round class="fl-r" @click="relationVisible = true" style="margin-left: 20px;"><i class="el-icon-plus" ></i> 添加拼团商品</el-button>
          </template>
          <el-button style="margin-left: 20px;" slot="right" size="small" type="primary" :disabled="!tableSelectionData.length" round class="fl-r" @click="batchStateVisible = true"><i class="el-icon-plus"></i> 批量更改状态</el-button>
          <el-button slot="right" size="small" type="primary" :disabled="!tableSelectionData.length" round class="fl-r" @click="batchSwitchVisible = true"><i class="el-icon-plus"></i> 批量切换分区</el-button>
        </gray-title>
        <br>
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      // 判断是否同一模块，带出搜索记录
      this.$Utils.filterSearchData(['admin/operation/extension/supper/assemble/configure', '/admin/operation/extension/supper/assemble/setting', '/admin/operation/extension/supper/assemble/details'], (res) => {
        this.formData = res
      })
    ...
    methods: {
      ...
      getListData () {
        ...
        this.$http.get('@ROOT_API/shareTheme/getDetail', {
          ...
          this.formData.goodsGroupSort = this.formData.goodsGroupSort || results.groupListPage.list.length && results.groupListPage.list[0].groupSort
     ...
     */
      ...
      handleLink (item, type) {
        ...
        if (type === 'setting') {
          ...
          localStorage.setItem(this.$global.SYSTEM + 'SupperSetting', JSON.stringify(item))
          localStorage.setItem(this.$global.FORM_DATA, JSON.stringify(this.formData))
          let url = '/admin/operation/extension/supper/assemble/setting'
          ...
          let url = '/admin/operation/extension/supper/assemble/details'
