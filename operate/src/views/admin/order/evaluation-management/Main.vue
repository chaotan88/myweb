...
<script>
  export default {
    ...
    methods: {
      getCommentPage: function (commentType) {
        // 获取订单列表
        this.search.commentType = commentType
      ...
      changeIsShow: function (commentId, isShow) {
        ...
        this.$http.post('/admin/comment/openComment', this.qs.stringify({
          ...
          if (res.data.res === 1) {
            this.getCommentPage(this.commentType) // 重新获取列表
          }
        })
      },
    ...
    watch: {
      '$route': function (to, from) {
        // 对路由变化作出响应...
        this.commentType = to.params.commentType
        this.getCommentPage(this.commentType)
      }
    },
    mounted: function () {
      this.commentType = this.$route.params.commentType
      this.getCommentPage(this.commentType)
    }
  }
</script>
