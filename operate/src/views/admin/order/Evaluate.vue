  ...
  <common-tpl class="manage-wrap" :footer="false">
    ...
    <template slot="main">
      ...
      <el-table border :data="tableData" style="width: 100%" v-loading="loading" element-loading-text="加载中">
        ...
        <el-table-column prop="customerName" label="买家姓名" min-width="100">
          <template slot-scope="scope">{{scope.row.customerName | filterEmpty}}</template>
        </el-table-column>
        <el-table-column prop="customerPhone" label="买家手机号" min-width="120">
          <template slot-scope="scope">{{scope.row.customerPhone | filterEmpty}}</template>
        </el-table-column>
        <el-table-column prop="commentType" label="评价类型" min-width="100">
          <template slot-scope="scope">
            {{scope.row.commentType | filterEvaluateStatus}}
          </template>
        </el-table-column>
        <el-table-column prop="orderCreateTime" label="评价时间" min-width="160">
          <template slot-scope="scope">
            {{scope.row.orderCreateTime | filterDate}}
          </template>
        </el-table-column>
        <el-table-column prop="commentContent" label="留言" min-width="200">
          <template slot-scope="scope">{{scope.row.commentContent | filterEmpty}}</template>
        </el-table-column>
        <el-table-column prop="isShow" label="是否显示" fixed="right" min-width="100">
          <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getListData (type) {
      let url = ''
      if (!type) {
        url = '@ROOT_API/comment/list'
      } else {
        url = 'comment/exportCommentList'
      }
      let data = {
     ...
     */
    handleDetails (item) {
      localStorage.setItem(this.$global.SYSTEM + 'EvaluateDetails', JSON.stringify(item))
