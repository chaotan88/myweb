<!-- 挂售管理 - 挂售记录  -->
<template>
  <common-tpl class="hang-sale-record-wrap"  footer >
    ...
    <template slot="main">
      <el-table border :data="tableData" class="full-w" v-loading="loading">
        ...
        <el-table-column prop="wholesalePrice" label="批发价" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.wholesalePrice | filterMoney | filterEmpty}}</div>
          </template>
        </el-table-column>
        <el-table-column prop="purchasePrice" label="抢购价" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.purchasePrice | filterMoney | filterEmpty}}</div>
          </template>
        </el-table-column>
<!--        <el-table-column prop="purchasePrice" label="会员预计收益" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.purchasePrice | filterMoney | filterEmpty}}</div>
          </template>
        </el-table-column>-->
        ...
        <el-table-column prop="hangDate" label="挂售时间" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.hangDate | filterDate}}</div>
          </template>
        </el-table-column>
        <el-table-column prop="sellDate" label="售出时间" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.sellDate | filterDate}}</div>
          </template>
        </el-table-column>
        <el-table-column prop="carryDate" label="提货时间" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.carryDate | filterDate}}</div>
          </template>
        </el-table-column>
        <el-table-column prop="cancelHangDate" label="取消挂售时间" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.cancelHangDate | filterDate}}</div>
          </template>
        </el-table-column>
<!--        <el-table-column prop="cancelHangDate" label="抵扣售出时间" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.cancelHangDate | filterDate}}</div>
          </template>
        </el-table-column>-->
        <el-table-column prop="status" label="挂售状态" min-width="120px">
          <template slot-scope="scope">
            <div>{{scope.row.status | filterHangSaleType}}</div>
          </template>
        </el-table-column>
        <div slot="empty">
          <no-data></no-data>
...
export default {
  ...
  data () {
    return {
      ...
      copyFormData: {},       // 拷贝表单数据
      userInfo: {},           // 用户信息
      pageType: 1,            // [ 抢购记录：1 详情-抢购记录：2 抢购管理-抢购记录]
      assembleData: {}        // 拼团列表数据
    }
  },
  ...
  mounted () {
    ...
    // 判断是否同一模块，带出搜索记录
    if (this.$route.path.match(/handSaleRecord/gi)) {
      this.$Utils.filterSearchData('operation/extension/handSale/handSaleRecord', (res) => {
        this.formData = res
      })
    }
  ...
  methods: {
    ...
    getListData (type) {
      let url = ''
      if (!type) {
        url = '@ROOT_API/purchase/getHangLogList'
      } else {
        url = 'purchase/exportHangLog'
      }
      let data = {
        params: {
