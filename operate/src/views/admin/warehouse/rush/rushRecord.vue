<!-- 抢购管理 - 抢购记录  -->
<template>
  <common-tpl class="rush-record-wrap"  footer >
    ...
    <template slot="main">
      <el-table border :data="tableData" class="full-w" v-loading="loading">
        <el-table-column prop="goodsNo" label="抢购订单编号" min-width="140">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.orderNo | filterEmpty}}</div>
          </template>
        </el-table-column>
        <el-table-column prop="goodsNo" label="抢购人手机号" min-width="120">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.phone | filterEmpty}}</div>
          </template>
        </el-table-column>
        <el-table-column prop="goodsName" label="抢购商品名称" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.goodsName | filterEmpty}}</div>
          </template>
        </el-table-column>
        <el-table-column prop="goodsName" label="抢购时间" min-width="160">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.grabDate | filterDate}}</div>
          </template>
        </el-table-column>
       <!-- <el-table-column prop="goodsNo" label="被抢购订单编号" min-width="140">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.goodsNo | filterEmpty}}</div>
          </template>
        </el-table-column>
        <el-table-column prop="goodsNo" label="被抢购人手机" min-width="140">
          <template slot-scope="scope">
            <div class="truncate">{{scope.row.goodsNo | filterEmpty}}</div>
          </template>
        </el-table-column>-->
        <el-table-column prop="goodsGroupPrice" label="抢购商品单价（元）" min-width="160px">
          <template slot-scope="scope">
        ...
        <el-table-column prop="status" label="抢购状态" min-width="120px">
          <template slot-scope="scope">
            <div>{{scope.row.status | filterRushAdmin}}</div>
          </template>
        </el-table-column>
        <el-table-column prop="goodsGroupPrice" label="获得批发券数" min-width="160px">
          <template slot-scope="scope">
...
export default {
  ...
  mounted () {
    ...
    // 判断是否同一模块，带出搜索记录
    if (this.$route.path.match(/rushDetailRecord/gi)) {
      this.pageType = 2
      this.formData.phone = localStorage.getItem(this.$global + 'LuckyBuyUserPhone')
    } else {
      this.$Utils.filterSearchData('warehouse/rush/rushRecord', (res) => {
        this.formData = res
      })
    }
  ...
  methods: {
    goBack () {
      let url = this.pageType === 2 ? '/admin/warehouse/rush/rushDetail' : '/admin/warehouse/rush/index'
    ...
    // getListData () {
    //   this.loading = true
    //   this.$http.get('@ROOT_API/grab/getGrabLogList', {
    //     params: {
    ...
    getListData (type) {
      let url = ''
      if (!type) {
        url = '@ROOT_API/grab/getGrabLogList'
      } else {
        url = 'grab/exportGrabLog'
      }
      let data = {
        params: {
          goodsId: this.formData.goodsId,
