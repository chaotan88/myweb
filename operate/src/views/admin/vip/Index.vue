    ...
    <template slot="main">
      <el-table :data="tableData" style="width: 100%" v-loading="loading" element-loading-text="加载中">
        ...
        <el-table-column prop="registerTime" label="注册时间" min-width="120">
          <template slot-scope="scope">{{scope.row.registerTime | filterDateDot}}</template>
        </el-table-column>
        <el-table-column prop="address" label="所在地" min-width="150">
        ...
        <el-table-column prop="lastLoginTime" label="最后登录时间" min-width="120">
          <template slot-scope="scope">{{scope.row.lastLoginTime | filterDateDot}}</template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getListData (type) {
      let url = ''
      if (!type) {
        url = '@ROOT_API/member/getMemberList'
      } else {
