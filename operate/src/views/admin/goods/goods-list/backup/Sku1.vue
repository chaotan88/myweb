      ...
      <el-form-item>
        ...
        <table class="spec-table sku-table">
          <tr>
            ...
            <th :class="{'vertical-line': index === 1}" v-for="(item, index) in skuTitle" width="110px">{{item}}</th>
            <th class="vertical-line" min-width="120px"><div>条形码</div></th>
            <th class="vertical-line ta-c" width="80px">当前库存</th>
            <th class="vertical-line" width="120px"><div :class="{'with-asterisk': examine === 1}">进货价</div></th>
            <th class="vertical-line" width="120px"><div :class="{'with-asterisk': examine === 1}">销售价</div></th>
            <th class="vertical-line" width="120px">利润率（利润/成交价）</th>
          ...
          <tr v-for="(item, index) in formData.specTableData" v-if="item.visible !== false">
            ...
            <td class="pd-r">
              <el-form-item :prop="'specTableData.' + index + '.buyingPrice'" :rules="ruleBuyPrice">
                <el-input v-model="item.buyingPrice" :disabled="examine === 2" @blur="inpBlur(index, 'buyingPrice')"></el-input>
              </el-form-item>
              <span title="一键同步" class="pos-a sync-btn">
            ...
            <td class="pd-r">
              <el-form-item :prop="'specTableData.' + index + '.goodsPrice'" :rules="ruleSalePrice">
                <el-input v-model="item.goodsPrice" :disabled="examine === 2" @blur="inpBlur(index, 'goodsPrice')"></el-input>
              </el-form-item>
              <span title="一键同步" class="pos-a sync-btn">
            ...
            <!-- 利润率 -->
            <td class="ta-c" v-if="item.goodsPrice && item.buyingPrice">
              <template v-if="item.whetherSpecial">{{((item.goodsSpecialPrice - item.buyingPrice) / item.goodsSpecialPrice * 100).toFixed(2) | filterEmpty('%')}}</template>
              <template v-else>{{((item.goodsPrice - item.buyingPrice) / item.goodsPrice * 100).toFixed(2) | filterEmpty('%')}}</template>
             <!--  <template v-if="item.goodsPrice && item.buyingPrice">{{((parseFloat(item.goodsPrice) - parseFloat(item.buyingPrice)) / parseFloat(item.goodsPrice) * 100).toFixed(2)}}%</template> -->
              <!-- item.goodsSpecialPrice -->
              <!-- <template v-if="item.goodsSpecialPrice && item.buyingPrice">{{((parseFloat(item.goodsSpecialPrice) - parseFloat(item.buyingPrice)) / parseFloat(item.goodsSpecialPrice) * 100).toFixed(2)}}%</template>
              <template >{{'' | filterEmpty}}</template> -->
            </td>
            <!-- 条形码 -->
            ...
            <td class="pd-l pd-r">
              ...
              <el-form-item :prop="'specTableData.' + index + '.goodsSpecialPrice'" :rules="rulePrice">
                <el-input v-model="item.goodsSpecialPrice" :disabled="examine === 2 || !item.whetherSpecial"  @blur="inpBlur(index, 'goodsSpecialPrice')"></el-input>
              </el-form-item>
              <span title="一键同步" class="pos-a sync-btn">
            ...
            <td class="pd-l pd-r">
              ...
              <el-form-item :prop="'specTableData.' + index + '.consumeGetPoint'" :rules="ruleGetPoint">
                <el-input v-model="item.consumeGetPoint" :disabled="!item.whetherGetPoint"></el-input>
              </el-form-item>
              <span title="一键同步" class="pos-a sync-btn">
...
export default {
  ...
  methods: {
     ...
     */
    getEditSpecList () {
      this.$http.get('@ROOT_API/standard/getStandardByGoodsId', {
        params: {
