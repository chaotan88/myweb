...
export default {
  ...
  data () {
    ...
    let validatePoints2 = (rule, value, callback) => {
      ...
      if (this.adjustmentSkuData.goodsSkuList[num].whetherGetPoint && !value) return callback(new Error('请输入自定义积分数量'))
      if (value && !value.toString().match(/^\d+$/g)) return callback(new Error('请输入正确的数值'))
      callback()
    }
    ...
    let validatePoints = (rule, value, callback) => {
      let num = parseInt(rule.field.split('.')[1])
      if (this.adjustmentSkuData.goodsSkuList[num].whetherPoints && !value) return callback(new Error('请输入积分兑换价格'))
      if (value && !value.toString().match(/^\d+$/g)) return callback(new Error('请输入正确的数值'))
      callback()
    }
