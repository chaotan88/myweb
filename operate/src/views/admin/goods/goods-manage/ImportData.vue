    ...
    <template slot="header">
      ...
      <high-search :textVisible="false">
        <div slot="search" class="upload-wrap">
          <dl class="d-ib va-m pos-r" style="padding-left: 80px;">
            <dt class="ta-c pos-a" style="border-radius: 18px 0 0 18px">浏览</dt>
            <dd><!--请选择导入文件-->
            </dd>
            <el-upload
              :action="uploadUrl"
              ...
              :file-list="fileList">
              <el-button size="small" type="primary" class="import-btn">请选择导入文件</el-button>
...
export default {
  ...
  computed: {
     ...
     */
    uploadUrl () {
      return this.$dm.ROOT_API + 'upload/uploadimg/malladmin/source?clientType=1&token=' + this.userInfo.token
    }
  },
  mounted () {
  ...
  methods: {
    ...
    uploadBefore (file) {
      ...
      if (this.beforeUpload) {
        ...
        const pattern = /(xls|xlss|sheet)$/ig
        const isLegalPhoto = pattern.test(file.type)
        const fileSize = file.size / 1024 / 1024 < 2

        if (!isLegalPhoto) {
          this.$message.error('上传文件格式不合法，只支持导入xls,xlsx 格式的文件')
        }
        if (!fileSize) {
