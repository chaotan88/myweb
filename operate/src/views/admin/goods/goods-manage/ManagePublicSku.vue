<!-- 商品管理 - sku -->
<template>
  <common-tpl class="pos-r goods-manage-public" footer back :collapse="true">
    <!-- 头部 -->
...
<!--        <gray-title title="商品参数：">
      ...
      <el-form :model="formData" :rules="rules" ref="formData" label-width="180px">
        ...
        <div class="commodity-details">
          <el-form-item label="选择供应商：" prop="supplierNewId" :rules="{required: true, message: '不能为空', trigger: 'change'}">
            <supplier-select text=" " :id="formData.supplierNewId" @change="supplierChange"></supplier-select>
          </el-form-item>
          ...
          <div v-show="formData.spec === 1">
            <sku-wrap ref="sku-wrap" :level="formData.goodsTypeLevel" :supplierId="formData.supplierNewId" :goodsTypeId="parseInt(platformAttributeData.small.id)" :spec="formData.spec" :assignSkuData="assignSkuData" @validate="handleSkuValidate"></sku-wrap>
          </div>
          ...
          <el-form-item label="详情描述：" prop="details" class="full-w">
            <editor v-model="formData.editorText" id="editorText" name="editorText" class="ueditorFlag" @change="ueditorChange" ref="deitor-first"></editor>
          </el-form-item>
         ...
         <!--       <td class="td-padding td-title">
            ...
            <el-form-item label="企业实地信息：" prop="details" class="full-w">
              <editor v-model="formData.enterpriseSpotImg" id="enterpriseSpotImg" name="enterpriseSpotImg" class="ueditorFlag" @change="ueditorChange2" ref="deitor-second"></editor>
            </el-form-item>
          </section>
    ...
    <template slot="collapse">
      <el-form :model="formData">
        ...
        <el-form-item label="是否上架：" prop="saleStatus" v-if="formData.reviewStatus === 1">
        ...
        <!--<el-form-item label="发货区域限制：" prop="fahou">
  ...
  methods: {
    ...
    getDetailsData () {
      this.$http.post('@ROOT_API/goods/getGoodsDetail', {
        ...
        if (detailsData.platformTypeGrandParentId) {                                 // 是否为第三层次
          this.formData.goodsTypeLevel = 3
        } else {
          this.formData.goodsTypeLevel = ''
        }
    ...
    getBrandListData (val) {
      this.$http.post('@ROOT_API/goods/getSupplierBrandList', {supplierId: val}).then((res) => {
        let resData = res.data
        if (parseInt(resData.status) !== 1) {
          this.$message({
            message: resData.msg,
    ...
    */
    ...
    submitForm (formName, type) {
      ...
      this.$refs[formName].validate((valid) => {
        ...
        // 默认规格
        let defaultSku = this.formData.skuResults.length === 1 ? this.formData.skuResults : this.formData.skuResults.filter(row => parseInt(row.whetherDefaultSku) === 1)
        let goodsPrice = ''
        let buyingPrice = ''
        let whetherSpecial = ''
        ...
        if (defaultSku && defaultSku[0]) {
          ...
          goodsPrice = defSku.whetherSpecial === 1 ? defSku.goodsSpecialPrice : defSku.goodsPrice
        ...
        //  goodsPrice: this.formData.spec === 1 ? this.formData.salePriceDefault : this.formData.salePrice,
          ...
          saleStatus: parseInt(this.formData.reviewStatus === 1 ? this.formData.saleStatus : 2),                  // 是否上架
    ...
    supplierChange (value) {
      this.formData.supplierNewId = value === '' ? null : value
      this.formData.brandId = ''
      this.getBrandListData(value)
      this.$refs['sku-wrap'].getSpecList(value)
    },
    /**
     * 分类选择
     */
