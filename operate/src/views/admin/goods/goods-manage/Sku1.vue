...
<!--              <template v-if="parseFloat(item.goodsPrice) > 0 && parseFloat(item.buyingPrice) > 0 && (!isNaN(item.goodsPrice)) && (!isNaN(item.buyingPrice)) && item.whetherSpecial === false">{{((parseFloat(item.goodsPrice) - parseFloat(item.buyingPrice)) / parseFloat(item.goodsPrice) * 100).toFixed(2)}}%</template>
        ...
        <!--  <el-table-column label="余额购买" prop="currentGoodsNum">
            <template slot-scope="scope">
          ...
          <el-table-column min-width="160px">
            ...
            <template slot-scope="scope">
              ...
              <el-row  v-if="formData.getIntegral === 1" type="flex">
                ...
                <!-- <el-checkbox v-model="scope.row.whetherGetPoint">
                 </el-checkbox>-->
              <!--  <span style="line-height: 40px;padding-left: 20px;">%</span>-->
              </el-row>
            </template>
          </el-table-column>
          <el-table-column min-width="160px">
...
export default {
  ...
  data () {
    ...
    let validatePoints2 = (rule, value, callback) => {
      ...
      if (this.formData.specTableData[num].whetherGetPoint && !value) return callback(new Error('请输入自定义积分数量'))
      if (value && !value.toString().match(/^\d+$/g)) return callback(new Error('请输入正确的数值'))
      callback()
    }
  ...
  watch: {
    ...
    'assignSkuData': {
      handler (value) {
        ...
        console.log(this.formData.specTableData)
        this.specChange()
      },
      deep: true
    }
