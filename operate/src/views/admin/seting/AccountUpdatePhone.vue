...
<template>
  <div class="update-phone-wrap">
    <template v-if="curStep === 1">
      <el-form :model="verifiData" :rules="rules" ref="verifiPhone" label-position="right" label-width='100px'>
        <el-form-item inline label='手机号：' :required="!userInfo.phone" prop="phone">
          ...
          <template v-else>
            <el-input class='inp-phone' placeholder='请输入手机号' v-model="verifiData.phone" :maxlength="11"></el-input>
          </template>
        </el-form-item>
        <el-form-item inline label='验证码：' prop='code'>
          <el-input class='inp-code' placeholder='输入4位数验证码' v-model="verifiData.code"></el-input>
          <el-button class='ta-c pos-a get-code-btn' v-if="initSendCode" @click="sendCodeHandle()">获得验证码</el-button>
    ...
    <template v-else>
      <el-form :model="verifiData" :rules="rules" ref="verifiPhone" label-position="right" label-width='100px'>
        <el-form-item inline label='手机号：' prop='newPhone' style="display: none;">
          <el-input ref='newPhone' placeholder='按手机格式填写（11位数字）' v-model="verifiData.newPhone" :maxlength="11"></el-input>
        </el-form-item>
        ...
        <el-form-item inline label='验证码：' prop='newCode'>
          ...
          <el-button class='ta-c pos-a get-code-btn' v-if="initSendCode" @click='sendNewCodeHandle(verifiData.newPhone)'>获得验证码</el-button>
          <el-button class='ta-c pos-a' disabled v-else>{{sendCodeNum}}s后重新获取</el-button>
        </el-form-item>
        <div class="ta-c next-btn-wrap">
...
export default {
  ...
  mounted () {
    ...
    this.curStep = this.userInfo.phone ? 1 : 2
  },
  methods: {
    /**
     * 倒计时
     ...
     */
    sendCodeHandle () {
      if (!this.userInfo.phone && !this.verifiData.phone) {
        this.$message({
          message: '请输入手机号',
          type: 'error',
          duration: 1500
        ...
        let rePhone = /^1\d{10}$/
        if (!this.userInfo.phone && (this.verifiData.phone.length !== 11 || !rePhone.test(this.userInfo.phone))) {
          this.$message({
            message: '请输入正确的手机号',
            type: 'error',
            duration: 1500
      ...
      // ajax发送验证码...
      this.sendCodePublicHandle(this.userInfo.phone)
    },
    ...
    sendNewCodeHandle (value) {
      ...
      // 发送验证码
      this.sendCodePublicHandle(this.verifiData.newPhone)
    },
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        ...
        this.$http.post('@ROOT_API/accountSetController/updatePhone', {
          ...
          this.userInfo.phone = this.verifiData.newPhone
