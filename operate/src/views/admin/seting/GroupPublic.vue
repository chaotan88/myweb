...
<template>
  <common-tpl class="add-group-wrap" footer back>
    <template slot="main">
      ...
      <el-form :model="formData" :rules="rules" ref="auth-form" label-width="240px">
        ...
        <el-form-item prop="selectList" class="table-item-wrap">
          ...
          <el-input v-model="formData.selectList.length" class="d-n"></el-input>
        </el-form-item>
      </el-form>
    </template>

    <template slot="footer">
      <el-button type="primary" @click="submitForm('auth-form')" :loading="loading">确定</el-button>
    </template>
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        selectList: [{type: 'array', required: true, validator: validateSelectList, trigger: 'change'}]
      },
      ...
      this.$http.get('@ROOT_API/auth/menus').then((res) => {
        ...
        this.filterGroupMenu(resData.data).then((res) => {
          ...
          setTimeout(() => {
            let disabledLen = 0
            let isCheckAll = true
            this.groupData.map((row, index) => {
              if (row.selectList.length !== row.childMenuList.length) isCheckAll = false
              if (row.disabled === true) disabledLen++
              if (row.id === 5036) {
                this.groupData.splice(2, 0, row)
                this.groupData.splice(this.groupData.length - 1, 1)
              }

              if (row.id === 5002) {
                row.childMenuList.forEach((item, itemIndex) => {
                  if (item.id === 5044) {
                    row.childMenuList.splice(0, 0, item)
                    row.childMenuList.splice(row.childMenuList.length - 2, 1)
                  }
                })
              }
            })
            ...
            setTimeout(() => {
              this.filterResults()
              this.$refs['auth-form'].clearValidate()
            }, 50)
          }, 30)
        })
      })
    ...
    submitForm (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        ...
        this.$http.post(url, {
          ...
          this.$router.push({path: '/admin/seting/authgroup'})
        })
      })
    }
  }
