<!-- 广告位设置 - 移动端 & PC端 -->
<template>
  <common-tpl class="adverti-public-wrap">
    <!-- 主体内容 -->
    <template slot="main">
      <el-table v-loading="loading" element-loading-text="加载中"
        ...
        <el-table-column prop="putLocation" label="投放位置">
          <template slot-scope="scope">
            <template v-if="parseInt(scope.row.applicationTerminal) === 1">{{scope.row.advertName}}</template>
        ...
        <el-table-column prop="advertPositionDetailList" label="广告位图片">
          <template slot-scope="scope">
            <template v-if="scope.row.advertPositionDetailList.length">
          ...
          <template slot-scope="scope">
            <el-dropdown class="d-b">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item>
                  <div @click="$router.push({path: '/admin/operation/adverti/edit', query: {id: scope.row.id}})"><i class="icon el-icon-edit"></i>管理</div>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
