  ...
  <common-tpl class="record-detail-wrap">
    ...
    <template slot="header" class="search-wrap">
      ...
      <high-search @search="highSearch" :textVisible="false">
        <div class="pos-r" slot="search">
          <el-input placeholder="输入会员姓名/会员手机" v-model="formData.condition"></el-input>
    ...
    <template slot="main">
      <el-table :data="tableData" style="width: 100%">
        <el-table-column prop="addSource" label="积分类型">
          <template slot-scope="scope">
            {{scope.row.addSource | filterSourceType}}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="rankName" label="会员身份"></el-table-column>
        <el-table-column prop="addTime" label="获得时间">
          <template slot-scope="scope">
      ...
      <div class="common-bottom-wrap handleup-b">
       <el-pagination background layout="prev, pager, next" :current-page="currentPage" :total="listTotal" @current-change="pageChange"></el-pagination>
      </div>
    </template>
  ...
  methods: {
    ...
    getListData () {
      ...
      this.$http.post('@ROOT_API/shopPointsController/getShopPointsDetailList', {
        ...
        shopId: parseInt(this.shopId),      // 店铺id
        ...
        this.listTotal = resData.data.totalPages        // 自定义好的列表总条数为0，需要传值给后台数据，循环后台传回来的数据
        // success...
      })
    },
