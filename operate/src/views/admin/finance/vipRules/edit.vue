        ...
        <el-form :model="formData" :rules="rules" ref="formData" label-width="220px" class="demo-formData">
          ...
          <div class="mb-20">
            ...
            <el-form-item label="拼团权益：" prop="groupProfitRate">
              <el-input v-model="formData.groupProfitRate" placeholder="0-100的数字,允许精确到百分位"  maxlength="6"></el-input><span class="ml-10">%</span>
              <!--<span class="ml-10 color-999">注：发起拼团后，参与拼团会员的产生的商品纯利润按拼团权益百分比分给上级直推会员的奖励</span>-->
            </el-form-item>
            <el-form-item label="团推推：" prop="groupRecommendProfitRate">
              <el-input v-model="formData.groupRecommendProfitRate" placeholder="0-100的数字,允许精确到百分位"  maxlength="6"></el-input><span class="ml-10">%</span>
             <!-- <span class="ml-10 color-999">注：直推奖，直推会员周期内拼团收益的百分点奖励给上级会员</span>-->
            </el-form-item>
            ...
            <el-form-item label="配送绿宝比例：" prop="greenGemstoneRate">
              <el-input v-model="formData.greenGemstoneRate" placeholder="0-100数字，允许精确到百分位" maxlength="100"></el-input>
              <span class="ml-10">%</span>&nbsp;&nbsp;推荐用户购买套餐后上级获得的绿宝，绿宝收益=套餐金额 X 套餐收益比例 X 配送绿宝比例
            </el-form-item>
            <el-form-item label="允许代言商品件数：" prop="allowAdvocateGoodsNum">
              <el-input v-model="formData.allowAdvocateGoodsNum" placeholder="10个字符，大于等于0的数字" maxlength="10"></el-input>
            </el-form-item>
            <el-form-item label="封顶数量：" prop="cappingNum">
              <el-input v-model="formData.cappingNum" placeholder="10个字符，大于等于0的数字" maxlength="10"></el-input>
<!--              <span class="ml-10">/周期</span>-->
            </el-form-item>
            <el-form-item label="代言商品收益：" prop="advocateGoodsProfitRate">
              <el-input v-model="formData.advocateGoodsProfitRate" placeholder="0-100数字，允许精确到百分位" maxlength="100"></el-input>
          ...
          <div class="mb-20">
            ...
            <div class="award">
              ...
              <!--</el-form-item>-->
              <el-form-item label="全球加权分红：" prop="globalWeightProfitRate">
                <el-row type="flex">
                  <el-input v-model="formData.globalWeightProfitRate" placeholder="0-100数字，允许精确到百分位" maxlength="100"></el-input><span class="ml-10">%</span>
                  <el-form-item label="特殊加权分红" prop="specialWeightProfitRate" label-width="120px">
                    <el-input v-model="formData.specialWeightProfitRate" placeholder="0-100数字，允许精确到百分位" maxlength="100"></el-input><span class="ml-10">%</span>
                  </el-form-item>
                </el-row>
              </el-form-item>
            </div>
...
<script>
  export default {
    data () {
      ...
      return {
        ...
        formData: {
          ...
          greenGemstoneRate: '',                     // 配送绿宝比例
          allowAdvocateGoodsNum: '',                    // 允许代言商品件数
          cappingNum: '',                    // 封顶数量
          advocateGoodsProfitRate: '',                    // 代言商品收益
          globalWeightProfitRate: '',                   // 全球加权分红
          specialWeightProfitRate: ''                   // 特殊加权分红
        },
        rules: {
          // 级别名称
          ...
          greenGemstoneRate: [{ required: true, validator: validatePercent, trigger: 'blur' }],
          allowAdvocateGoodsNum: [{ required: true, validator: validateNumberWithVoid, trigger: 'blur' }],
          cappingNum: [{ required: true, validator: validateNumberWithVoid, trigger: 'blur' }],
          advocateGoodsProfitRate: [{ required: true, validator: validatePercent, trigger: 'blur' }],
          globalWeightProfitRate: [{ required: true, validator: validatePercent, trigger: 'blur' }],
          specialWeightProfitRate: [{ required: true, validator: validatePercent, trigger: 'blur' }]
        }
      }
    },
    ...
    methods: {
       ...
       */
      getDataDetail () {
        this.$http.get('@ROOT_API/buyGroupCommissionSetController/getRuleDetail', {
          params: {
      ...
      submitForm (formName) {
        this.$refs[formName].validate((valid) => {
          ...
          this.$http.post('@ROOT_API/buyGroupCommissionSetController/updateGroupRankSet', {
            ...
            greenGemstoneRate: parseFloat(this.formData.greenGemstoneRate),                   // 配送绿宝比例
            allowAdvocateGoodsNum: parseInt(this.formData.allowAdvocateGoodsNum),                   // 允许代言商品件数
            cappingNum: parseInt(this.formData.cappingNum),                   // 封顶数量
            advocateGoodsProfitRate: parseFloat(this.formData.advocateGoodsProfitRate),                   // 代言商品收益
            globalWeightProfitRate: parseFloat(this.formData.globalWeightProfitRate),                  // 全球加权分红
            specialWeightProfitRate: parseFloat(this.formData.specialWeightProfitRate)                   // 特殊加权分红
          }).then((res) => {
            ...
            setTimeout(() => {
              this.$router.push('/admin/finance/commission/vipRules/index')
            }, 300)
          }).finally(() => {
            setTimeout(() => {
