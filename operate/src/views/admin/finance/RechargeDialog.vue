    ...
    <el-form label-position="right" label-width="220px" class="ta-l" :model="formData" :rules="rules" ref="form">
      ...
      <div class="form-item-group">
        ...
        <el-form-item :label="descriptionTitle" class="full-w">
          ...
          <template v-else>{{initData.descriptionSub}}</template>
        </el-form-item>
...
export default {
  ...
  data () {
    ...
    let validateRechargeBalance = (rule, value, callback) => {
      ...
      if (this.pageType === 1 && parseFloat(value) <= 0 && !this.formData.rechargeIntegral && !this.formData.rechargeCash) return callback(new Error('金额或消费积分或通用积分至少填一个'))
    ...
    let validateRechargePublic = (rule, value, callback) => {
      ...
      if (this.pageType === 1 && !value && !this.formData.rechargeIntegral && parseFloat(this.formData.rechargeBalance) <= 0 && !this.formData.rechargeCash) return callback(new Error('金额或消费积分或通用积分至少填一个'))
      if (this.actionType === 2 && this.pageType === 1) {
        if (rule.field === 'rechargeIntegral' && Math.abs(this.formData.rechargeIntegral) > parseInt(this.initData.bonusPoints) || rule.field === 'rechargeCash' && Math.abs(this.formData.rechargeCash) > parseInt(this.initData.cashPoints)) return callback(new Error('扣除值不能大于当前余额'))
      }
      if (!this.checkFlag.balance) {
    ...
    return {
      ...
      rules: {
        ...
        voucherImgStr: [{required: true, validator: validateVoucherImg, trigger: 'change'}],
  ...
  methods: {
    ...
    getMemberInfo () {
      this.$http.post('@ROOT_API/accountRechargeController/getMemberInfo', {
        ...
        this.formData.rechargeCash = results.cashPoints
      })
    },
    ...
    handleSubmit () {
      ...
      if (this.pageType === 2) {
        ...
        cashPoints = this.formData.rechargeCash || 0
      }
      this.$http.post(url, {
