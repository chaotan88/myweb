        ...
        <popup v-model="attributeType" position="bottom" height="75%" @on-hide="ClosePop">
          <div class="attributeType-box" style="border-top:1px solid #fff;">
              ...
              <div class="goodInfor-box">
                <ul class="goodInfor-ul">
                  ...
                  <li class="right">
                    ...
                    <!-- 正常商品 -->
                    <template v-if="orderSource != 10 && orderSource != 9">
                      <span class="price">¥{{goodsPriceStock.price | moneyFilter}}</span>
                      <!--price-->
                    </template>
                    <template v-if="orderSource == 10 || orderSource == 9">
                      <span class="price">
                        ¥{{luckyData.purchasePrice | moneyFilter}}
                         <span class="icon" v-if="orderSource == 10">
                         {{'+ ' +luckyData.luckyAddPrice}} 批发券
                        </span>
                        <span class="iconLine" style="text-decoration: line-through;
                  color:#999999;margin-left: .1rem;font-size: .24rem" v-if="orderSource == 9">
                         {{'¥ ' +luckyData.goodsPrice}}
                        </span>
                      </span>
                    <!--price-->
                    </template>
                    <!--赠送积分 福利专区拼团不显示-->
                    <!--<div class="exchangePoints" v-show="">-->
                      <!--<div>-->
                        <!--<span>赠送</span>-->
            ...
            <div class="attrStyle" id="attrStyle">
              ...
              <!--数量-->
              <ul class="num-box clr" v-show="orderSource != 9 && orderSource != 10 ">
                <li class="left">数量 {{orderSource}}</li>
                <li class="right">
                  <com-cartcontrol  v-model="goodsNum" :max="goodsPriceStock.stock"></com-cartcontrol>
                </li>
              </ul>
              <ul class="num-box clr odr" v-show="orderSource == 9 || orderSource == 10">
                <li class="left" v-if="orderSource == 9">购买数量</li>
                <li class="left" v-if="orderSource == 10">批发数量</li>
                <li class="left num" style="display: flex;flex-direction:row ">
                  <com-cartcontrol  v-model="goodsNum" :max="maxGoodsNum"></com-cartcontrol>
                  <span style="color: #999;font-size: .24rem;margin-left: .2rem" v-show="orderSource == 9">
                  允许购买数: {{ luckyData.luckyPurchaseNumLimit - luckyData.luckyUserTodaySnatchCount}}
                  </span>
                  <span style="color: #999;font-size: .24rem;margin-left: .2rem" v-show="orderSource == 10 && luckyData.myPurchaseCoupon >= luckyData.luckyAddPrice * goodsNum">
                  允许购买数: {{ luckyData.luckyPurchaseNumLimit - luckyData.luckyUserTodaySnatchCount}}
                  </span>
                  <span style="color: #e64340;font-size: .24rem;margin-left: .2rem" v-show="orderSource == 10 && luckyData.myPurchaseCoupon < luckyData.luckyAddPrice * goodsNum">
                  批发券不足,<span @click="goPage()" style="color: #10aeff">去获取</span>
                  </span>
                </li>
                <li class="clear" >
                  <div class="Ic" v-if="orderSource == 9">
                    <div>
                      <div class="iconFor">
                        <span> <img src="../../assets/images/shopLucky/quan2.png"/></span>
                        <span class="iocnText">赠</span>
                        {{luckyData.luckyAddPrice * goodsNum}} 元批发券</div>
                    </div>
                    <div class="text">
                      拥有此券可兑换商品的销售权益
                      <!--<span class="fillicon" v-show="check"><img src="../../assets/images/goodsDietail/check.png"/></span>-->
                      <!--<span class="fillicon" v-show="!check"><img src="../../assets/images/goodsDietail/deft.png" /></span>-->
                      <!--<span>立即采购</span>-->
                      <div>
                      </div>
                    </div>
                  </div>
                  <div  v-if="orderSource == 10">
                    <p class="clearText">
                      批发金: <span>{{luckyData.purchasePrice * goodsNum | moneyFilter}}</span>, 批发券: <span>{{luckyData.luckyAddPrice * goodsNum }}</span>, 出售获得: <span>{{luckyData.hangRebate * goodsNum | moneyFilter}}</span>
                    </p>
                  </div>
                </li>
            ...
            <div class="bottom">
              ...
              <div class="btn" @click="popConformBtn(typeNum)" v-show="basicGoodsInfor.activeType == 1 && ! gostatus">{{orderSource == 9 ? '立即抢购' : '立即批发'}}</div>
              <div class="btn btnRrorr" @click="ClosePop()"  v-show="basicGoodsInfor.activeType != 1 || gostatus">{{orderSource == 9 ? '立即抢购' : '立即批发'}}</div>
            </div>
          </div>
        </popup>
      </div>
...
//        } else {
//          if (this.coyPrice) {
//            this.goodsPriceStock.price = this.coyPrice
//          }
//        }
      },
      /**
       ...
       */
      oderDateFn (res) {
        if (!res.luckyData) return false
        var time = res.luckyData.startDate - res.luckyData.luckySysTime
        var end = res.luckyData.endDate - res.luckyData.luckySysTime
        this.maxGoodsNum = res.luckyData.luckyPurchaseNumLimit - res.luckyData.luckyUserTodaySnatchCount
        if (res.luckyData.luckyPurchaseNumLimit <= 0) {
          this.goodsNum = 0
        }
//        activeType  1:未开始 2:开始 3;结束
        ...
        if (this.activeTypeData) {
          res.activeType = this.activeTypeData
        }
        if (res.luckyData.typeLucky === 10 && res.luckyData.myPurchaseCoupon <= 0 && res.luckyData.isLogin === 1) {
          this.gostatus = 10
        }
        if (res.luckyData.typeLucky === 10 && res.luckyData.myPurchaseCoupon < res.luckyData.luckyAddPrice && res.luckyData.isLogin === 1) {
          this.gostatus = 10
        }
        let resData = {
          data: res
