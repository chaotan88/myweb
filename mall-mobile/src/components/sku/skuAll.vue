                ...
                <ul class="goodInfor-ul">
                  ...
                  <li class="right">
                    ...
                    <template v-if="!pageType || typeNum !== 4 || (pageType === 1 ? !goodsPriceStock.welfarewZoneData.whetherBonusPrice : pageType === 2 ? !goodsPriceStock.welfarewZoneData.whetherLeaguePrice : pageType === 3 ? !goodsPriceStock.welfarewZoneData.whetherGoldBeanPrice : '')">
                      <span class="price">¥{{(typeNum == 7 || typeNum == 13) ? goodsPriceStock.goodsGroupPrice : typeNum == 12 ? '0.01' : goodsPriceStock.price | moneyFilter}}</span>
                      <!--price-->
                    </template>
                    <!-- 福利专区 -->
                    ...
                    <div class="exchangePoints" v-if="cangetPoints && typeNum !== 4">
                  ...
                  <div class="del-box" @click="ClosePop">
...
<script>
  ...
  export default {
    ...
    props: [
      'groupId',                   // 拼团id
      'goodsDetail',
      'pageType',                  // sku展示类型  12超级拼团
      'skuOrderSource',
      'skuGoodsId',               // 订单信息
      'show',                  // 弹框信息
      'activeTypeData',         // 活动状态
      'pageType',
      'typeNum'                // 商品下单类型(OrderSource) 规格=1 购物车=2 立即购买=3 发起拼单=7 幸运大抢购=9 12参发起超级拼团 12超级拼团 13参加超级拼团
    ],
    data () {
      return {
        ...
        orderSource: this.skuOrderSource,        // 1:普通购物，2:分享订单,3:芯豆兑换,4:消费积分购买,5:联盟积分购买,6:一元购,7:拼团，8:限时抢购 9:抢购 10:批发 11:批发提货 12:超级拼团 13:参加超级拼团
...
//      console.log(this.basicGoodsInfor)
    ...
    methods: {
      ...
      ClosePop () {
//        console.log('关闭')
        this.$emit('sukBackByGroup', false)
      },
      /**
       * 判断库存是否够
       */
      ...
      popConformBtn (typeNum) {
        ...
        // 拼团不考虑 库存
        if (typeNum === 7 || typeNum === 12 || typeNum === 13) {
          this.confirmOrder()
          return false
        }
