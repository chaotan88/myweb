  ...
  <div>
    <x-dialog v-model="showPhonePop" class="dialog-demo" hide-on-blur @on-hide="closeModifyPhone"  :dialog-style="{'text-align': 'left', width: '80%', 'max-width': '80%'}">
      <div class="look-phonepop">
        ...
        <x-button type="warn" class="modify-btn" @click.native="saveData" v-else :disabled="!noTap">保存</x-button>
      </div>
    </x-dialog>
  </div>
</template>
      ...
      getCode (types) {
        if (types === 1) {
          ...
          if (this.newPhone === '') {
            ...
            return false
          } if (!this.newPhone.match(/^1\d{10}$/g)) {
        ...
        this.checkCode(() => {
          this.$http.post(`${this.api.modifyPhone}`, {
            ...
            if (res.data.status === this.api.ERR_OK) {
              ...
              this.$parent.modifyPhoneData(this.newPhone)
            }
            this.$vux.toast.text(res.data.msg)
          }).finally(() => {
            this.$vux.loading.hide()
