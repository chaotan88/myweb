<template>
  <div class="pay">
    ...
    <ol class="payType-box">
      <li class="clr" :class="(orderAmountInfo.orderTotalAmount >= limitMoney && v.type!=3 && v.type!=4) ? 'noPay' : ''" v-for="(v,index) in arr" @click="selectedPayFun(v)">
        <div class="left">
        ...
        <div class="mid">
          {{v.name}}
          <span v-if="v.type==4">（可用余额：10,000.00)</span>
        </div>
        <div class="right">
    ...
    <vue-pay-keyboard ref="pay" :is-pay='isPay' @close='isPay=false' @pas-end='pasEnd' :payStatusText="payStatusText" pay-title="身份认证"> </vue-pay-keyboard>
    <!--提交按钮-->
...
<script>
  ...
  import Vue from 'vue'
  import vuePayKeyboard from '../../../node_modules/vue-pay-keyboard/src/lib'
  ...
  export default {
    ...
    data () {
      return {
        ...
        arr: [], // 支付信息
        noOfflinePay: [
          {'checked': true, type: 1, name: '微信支付', noPay: ''},
          {'checked': false, type: 2, name: '支付宝支付', noPay: ''},
          {'checked': false, type: 4, name: '余额支付'}
        ],
        hasOfflinePay: [
          {'checked': false, type: 1, name: '微信支付', noPay: ''},
          {'checked': false, type: 2, name: '支付宝支付', noPay: ''},
          {'checked': true, type: 3, name: '线下支付', noPay: ''},
          {'checked': false, type: 4, name: '余额支付', noPay: ''}
        ],
    ...
    methods: {
        ...
        this.$http.get(resUrl, {
          ...
          if (resData.status === this.api.ERR_OK) {
            ...
            if (this.orderAmountInfo.orderTotalAmount >= this.limitMoney) {
              this.arr = this.hasOfflinePay
              this.currentPayType = 3
              
            } else {
              this.currentPayType = 1
              this.arr = this.noOfflinePay
            }
            // 授权后自动支付
      ...
      */
      selectedPayFun (v) {
        if (this.orderAmountInfo.orderTotalAmount >= this.limitMoney && v.type !== 3 && v.type !== 4) {
      ...
      */
      goPay () {
        if (this.currentPayType === 1) {
          ...
          this.$router.push({path: '/offlinePay'})
