...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      */
      judgeIfSetmeat () {
        this.$http.post(this.api.judgeIfSetmeat, JSON.parse(localStorage.getItem('goodsIdsArr'))).then((res) => {
          let resData = res.data
          if (resData.status === this.api.ERR_OK) {
            if (resData.data === 0) {
      ...
      isWechat () {
        this.$http.post(this.api.wxPay, {
          ...
          if (resData.status === this.api.ERR_OK) {
            ...
            // 授权
            window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx29ed774e4f21daca&redirect_uri=http%3A%2F%2F120.77.210.28%2Fbehindshop%2FwxAuthor%2Fdefault&response_type=code&scope=snsapi_base&state=${this.token}&connect_redirect=1#wechat_redirect`
          }
        })
      },
      /**
      ...
      */
      offlinePay () {
        this.$http.get(this.api.offlinePay, {
          ...
          if (res.data.status === this.api.ERR_OK) {
            this.$router.replace({path: '/paySuccess', query: {offlinePay: 'offlinePay'}})
          }
          this.$vux.toast.text(res.data.msg)
        })
      },
        ...
        this.$http.post(this.api.balancePay, {
          ...
          setTimeout(() => { // 模拟请求接口验证密码中
            if (resData.status === this.api.ERR_OK) {
              ...
              window.location.replace(window.location.href.match(/.*mshop/gi)[0] + '/paySuccess?shopId=1')
            } else {
