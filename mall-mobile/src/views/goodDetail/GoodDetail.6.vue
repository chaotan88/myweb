    ...
    <div class="goodDetail" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="10" infinite-scroll-immediate-check="false">
      ...
      <div class="infor-box">
        ...
        <div class="infor-price">
          <span v-if="basicGoodsInfor.goodsSpecialPrice || basicGoodsInfor.goodsPrice">¥{{basicGoodsInfor.goodsStandardList.length > 0 ? basicGoodsInfor.goodsPrice : basicGoodsInfor.goodsSpecialPrice ? basicGoodsInfor.goodsSpecialPrice : basicGoodsInfor.goodsPrice | moneyFilter }}</span>
      ...
      <div class="shopGetPoint" v-if="goodsPriceStock.points">积分: 购买此商品可获得{{goodsPriceStock.points}}积分</div> 
      ...
      <ol class="footer-box" :class="[{'footer-index':(typeNum==1)}]">
        ...
        <template v-if="typeNum == 1">
          <li class="conter" @click="popConformBtn(2)">加入购物车</li>
          <li class="right" @click="popConformBtn(3)">立即购买</li>
        </template>
        <!--购物车、立即购买弹窗-->
        <template v-else>
      ...
      arrValCombinat (types, attrId, isActive) {
        ...
        if (basicGoodsInfor.goodsStandardList.length > 0) {
          ...
          basicGoodsInfor.goodsSkuList.forEach((item, ind) => {
            console.log(item.standardValIdArr)
            // 不是组合属性
            if (basicGoodsInfor.goodsStandardList.length <= 1) {
              overplus.push(item)
            } else {   // 是组合属性
              attrId.forEach((ite) => {
                if (item.standardValIdArr.indexOf(ite) > -1) {
                  overplus.push(item)
                }
              })
            }
                  ...
                  attrId.forEach((ite) => {
                    overplus.forEach((el) => {
                      ...
                      if (basicGoodsInfor.goodsStandardList.length <= 1) {
                        ...
                        if (el.standardValIdArr.indexOf(ite) > -1) {
                          index = el.standardValIdArr.indexOf(ite)
          ...
          basicGoodsInfor.goodsSkuList.forEach((item, index) => {
            if (item.goodsSkuValList.length > 0) {
              ...
              if (activeLen.length === len) {
                ...
                this.goodsPriceStock = {
                  price: basicGoodsInfor.goodsPrice,
          ...
          this.goodsPriceStock = {
            price: basicGoodsInfor.goodsSkuList[0].goodsSpecialPrice ? basicGoodsInfor.goodsSkuList[0].goodsSpecialPrice : basicGoodsInfor.goodsSkuList[0].goodsPrice,
            stock: basicGoodsInfor.totalStock,
