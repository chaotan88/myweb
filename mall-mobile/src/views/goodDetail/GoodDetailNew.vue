<template>
    <div class="goodDetail">
      ...
      <div v-show="showContainer === 1" class="baseInfo" id="baseInfo">
        ...
        <div class="infor-box">
          ...
          <div class="price-wrap">
            ...
            <div class="normal-price" v-if="!pageType">
            ...
            <div class="actiy-price" v-else>
              <div class="price-num">
                <div>
                  <p class="num-tag">{{pageType | filterPointsTxt}}不足</p>
                  <p class="num-p">¥200+1000{{pageType | filterPointsTxt}}</p>
                </div>
                <div>原价: 200元</div>
              </div>
              <div class="price-save">省200元</div>
            </div>
        ...
        <!-- 说明 -->
        <div class="Explain" @click="showExplain" v-if="!pageType">
          <div>说明: </div>
          <ul>
      ...
      <!-- 福利区进入 -->
      <ul class="welfare-footer-box" v-if="pageType">
        <li class="fullBuy">全额购买</li>
        <li class="welfareBuy">{{pageType | filterbuyBtnTxt}}</li>
      </ul>
      <!-- 普通详情页显示 -->
      <ul class="footer-box" :class="[{'footer-index':(typeNum==1)}]" v-else>
        <li class="left">
          <!--未收藏-->
      ...
      <!-- 分享提示 -->
      <div class="share-box" v-if="showShareBg" @click="showShareBg = false">
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        paramerTabList: [
          ...
          {
            name: '溯源信息',
            component: 'goodsTraceability'
          }
        ],
        pageType: '',                             // 1 消费积分区 2 联盟积分区 3 芯豆区
    ...
    mounted: function () {
      ...
      this.pageType = this.$route.query.pageType                                   // 页面类型 (1 消费积分区 2 联盟积分区 3 芯豆区)
      ...
      confirmOrder () {
        ...
        this.$http.post(`${this.api.getBuySoon}`, {
          ...
          if (res.data.status === this.api.ERR_OK) {
            ...
            resData.orderSource = 1       // 1:普通购物，2：分享订单
            ...
            localStorage.setItem('CONFIRM_ORDER_TYPE', 'NORMAL')                           // 下单的类型
            setTimeout(() => {
