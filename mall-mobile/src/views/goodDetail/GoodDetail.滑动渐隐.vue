    ...
    <div class="goodDetail">
      ...
      <div class="detail-header-box" >
        ...
        <div class="detail-header" >
          <div class="detail-header-back" @click="$router.back()" ref="detailBack" id="detailBack" :style="{background: 'rgba(0,0,0,' + (0.3 - showDetailHeader/1.5 ) + ')'}">
            <x-icon type="ios-arrow-left" size="20" :style="{fill: 'rgba(255,255,255,' + (showDetailHeader ?1 - (showDetailHeader*1.5 + 0.3) : 1 ) + ')'}" v-if="(showDetailHeader*1.5 + 0.3) <= 0.8"></x-icon>  
            <x-icon type="ios-arrow-left" size="20" style="transition: all 1s" :style="{fill: 'rgba(248,42,42,' + (showDetailHeader*1.5 + 0.3) + ')'}" v-if="(showDetailHeader*1.5 + 0.3) > 0.8"></x-icon>  
          </div>  
          <div class="detail-header-blank"></div>  
          <div class="detail-header-option" :style="{opacity:showDetailHeader ?  showDetailHeader*1.5 + 0.3 > 1 ? 1 : showDetailHeader*1.5 + 0.3 : showDetailHeader}">
            <ul>
              <li :class="headTabIndex == index ? 'active' : ''" @click="headTabChange(index, item.component)" v-for="(item, index) in headTabList">{{item.name}}</li>
            </ul>
          </div>
          <div class="detail-header-home" @click="$router.push('/')" :style="{background: 'rgba(0,0,0,' + (0.3 - showDetailHeader ) + ')'}">
            <svg class="icon" aria-hidden="true">
              <use xlink:href="#icon-shouye"></use>
            </svg>
          </div> 
          <div class="detail-header-share" :style="{background: 'rgba(0,0,0,' + (0.3 - showDetailHeader ) + ')'}">
            <svg class="icon" aria-hidden="true">
  ...
  export default {
    ...
    mounted: function () {
      ...
      baseInfo.onscroll = () => {
        ...
        let as = s / 300 > 1 ? 1: s / 300
        // this.$refs.detailBack.style.opacity = op - as
        this.showDetailHeader = as
        // let adg = op - as
        // console.log(adg)
        // document.getElementById('detailBack').style.background = 'rgba(0,0,0,'+ adg +')'
