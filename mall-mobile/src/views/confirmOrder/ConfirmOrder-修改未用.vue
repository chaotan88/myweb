      ...
      <ul class="goodList-box">
        <div class="goodList-grop" v-for="(item, ind) in goodsList">
          ...
          <div class="sendType">
            ...
            <div class="right" v-if="item.totalPrice > freeAdmission.freeShipping">免运费</div>
            <div v-else @click="showLogisticPop(item.shopId, ind)" class="right">
              <span>
                <i v-if="JSON.stringify(item.selectLogistics) == '{}'">请选择配送方式</i>
                <i v-else>{{item.selectLogistics.name}} {{item.selectLogistics.price}}元</i>
              </span>
    ...
    computed: {
      ...
      */
      Total () {
        ...
        return parseFloat(this.goodstotalPrice) + logisticsPrice
    ...
    methods: {
      ...
      */
      confirmOrder () {
        ...
        this.goodsList.forEach((item) => {
          if (item.totalPrice < this.freeAdmission.freeShipping) {
            if (JSON.stringify(item.selectLogistics) === '{}') {
