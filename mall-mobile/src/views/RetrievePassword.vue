  ...
  <div class="retrieve-password-wrap">
    ...
    <template v-if="curStep == 1">
      <div class="ta-c inp-code-wrap">
        <div class="ta-c sended-text">我们已向<span class="d-ib phone">{{formData.phone || forgetPasswordData.phone}}</span>发送验证码短信，请查看短信并输入验证码</div>
...
export default {
  ...
  data () {
    return {
      ...
      forgetPasswordData: ''      // 忘记密码保存的数据
    }
  },
  mounted () {
    this.forgetPasswordData = JSON.parse(localStorage.getItem('forgetPasswordData'))
    this.curStep = this.$route.query.curStep ? this.$route.query.curStep : 0
  },
  methods: {
    /**
     * 校验验证码
    ...
    */
    checkPhoneExist (fun) {
      this.$http.post(this.api.checkPhoneExist, {
        ...
        if (resData.status === '9905') {
          fun()
        } else if (resData.status === this.api.ERR_OK) {
          this.$vux.toast.text('用户未注册')
          return false
        } else {
    ...
    handlevValidatePhone () {
      ...
      this.checkPhoneExist(() => {
        localStorage.setItem('forgetPasswordData', JSON.stringify({
          phone: this.formData.phone
        }))
        this.handleSendCode()
      })
    ...
    handleSendCode () {
      ...
      this.$http.post(this.api.getCode, {
        phone: this.formData.phone || JSON.parse(localStorage.getItem('forgetPasswordData')).phone
      }).then((res) => {
        let resData = res.data
        this.$vux.toast.text(resData.msg)
        if (resData.status === this.api.ERR_OK) {
          this.$router.push({query: {curStep: 1}})
        }
      })
    },
    ...
    handlevValidateCode () {
      if (this.inputCodeSuccess) {
        this.$http.post(`${this.api.checkCode}`, {
          phone: JSON.parse(localStorage.getItem('forgetPasswordData')).phone,
          ...
          if (res.data.status !== this.api.ERR_OK || !res) {
            ...
            localStorage.setItem('forgetPasswordData', JSON.stringify({
              phone: this.forgetPasswordData.phone,
              code: this.formData.code
            }))
            this.$router.push({query: {curStep: 2}})
          }
        })
      }
    },
    ...
    handlevValidatePassword () {
      ...
      this.$http.post(`${this.api.resetPassword}`, {
        ...
        verifyCode: this.forgetPasswordData.code
      }).then((res) => {
        let resData = res.data
