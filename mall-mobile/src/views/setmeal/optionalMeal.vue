      ...
      <div class="optionalMeal-content-r" v-infinite-scroll="loadMoreData" infinite-scroll-disabled="busy" infinite-scroll-distance="10" infinite-scroll-immediate-check="false">
        ...
        <ul>
          ...
          <li v-for="(item, index) in allDataList[tabBarIndex]" @click="$router.push({path: '/goodDetail', query: {goodsId: item.id}})">
            <!--白色遮罩-->
            <div class="Mask" v-if="item.totalStock === 0 || !item.totalStock"></div>
            <div class="optionalMeal-list-l">
              <img :src="item.mainImage | filterImgUrl">
              <div class="soldOut" v-if="item.totalStock === 0 || !item.totalStock">无货</div>
            </div>
            <div class="optionalMeal-list-r">
              ...
              <div class="optionalMeal-list-r-bot">
                <div class="optionalMeal-list-r-bot-price">¥{{item.goodsPrice | moneyFilter}}</div>
      ...
      <popup v-model="attributeType" position="bottom" height="75%" @on-hide="ClosePop">
        <div class="attributeType-box" style="border-top:1px solid #fff;">
          <div class="fixedBox">
            ...
            <div class="goodInfor-box">
              <ul class="goodInfor-ul">
                ...
                <li class="right">
                  <p class="goods-name">{{popUseData.goodsName}}</p>
          ...
          <ul class="num-box clr">
            <li class="left">数量</li>
            <li class="right">
              <com-cartcontrol  v-model="popInpVal" :max="goodsPriceStock.stock"></com-cartcontrol>
            </li>
          </ul>
          </div>
          <!--按钮-->
    ...
    methods: {
      ...
      */
      arrValCombinat (types, attrId, isActive) {
        ...
        if (popUseData.goodsStandardList.length > 0) {
              ...
              if (activeLen.length === len) {
                if (item.standardValIdArr.toString() === standardValIdArrs.toString()) {
                  this.goodsPriceStock = {
                    ...
                    isPoints: item.points ? !!true : !!false
                  }
                  this.skuId = item.id
                ...
                this.goodsPriceStock = {
                  price: popUseData.goodsMinPrice,
          ...
          let bPrice = popUseData.goodsSkuList[0]
          this.goodsPriceStock = {
            ...
            isPoints: popUseData.goodsSkuList[0].points ? !!true : !!false
          }
          this.skuId = popUseData.goodsSkuList[0].id
          this.getPointsByGoodsSku()   // 计算能获得多少积分
        }
          ...
          if (v.goodsStandarValList.length > 0) {
            ...
            if (index === 0) {
              v.goodsStandarValList.forEach((ite) => {
                ...
                let lenArr = []
                let lenNoTapArr = []
                this.popUseData.goodsSkuList.forEach((its) => {
      ...
      RightData (columnId) {
        ...
        this.$http.post(this.api.getSetmealGoodsListByColumnId, {
          ...
          res.data.data.list.forEach((item) => {
            ...
            item.goodsPrice = item.whetherSpecial && item.goodsSpecialPrice ? item.goodsSpecialPrice : item.goodsPrice      // 初始化价格
          })
          // 是否点击
          if (this.cover) {
      ...
      goodsNumIncrease (index, types) {
        if (types === 1) {
          ...
          if (!this.allDataList[this.tabBarIndex][index].goodsNum) {
            ...
            if (this.allDataList[this.tabBarIndex][index].goodsNum >= this.allDataList[this.tabBarIndex][index].maxLen) {
              this.allDataList[this.tabBarIndex][index].goodsNum = this.allDataList[this.tabBarIndex][index].maxLen
              return false
            }
      ...
      */
      cartcontrolInp (val, item) {
        ...
        if (item.goodsNum >= item.maxLen) {
          item.goodsNum = item.maxLen
          return false
        }
        this.calculatPrice()
      },
      ...
      */
      goodsNumReduce (index, types) {
        if (types === 1) {
          ...
          // 重置最小价格
          if (this.allDataList[this.tabBarIndex][index].goodsNum <= 0) {
            this.allDataList[this.tabBarIndex][index].goodsPrice = this.allDataList[this.tabBarIndex][index].goodsMinPrice
          }
          this.calculatPrice()
        } else if (types === 2) {
          if (this.popInpVal > 1) {
      ...
      */
      popConfirmBtnTap () {
        ...
        singList.goodsPrice = this.goodsPriceStock.price
        this.$set(singList, 'maxLen', this.goodsPriceStock.stock)
        this.$set(singList, 'selectedPrice', this.goodsPriceStock.price)
        this.calculatPrice()
      },
      /**
       * 关闭弹窗
      ...
      */
      calculatPrice () {
        ...
        this.allDataList.forEach((item) => {
          item.forEach((ite) => {
            if (ite.goodsNum) {
              totalPrice += (ite.whetherSpecial && ite.goodsSpecialPrice ? ite.goodsSpecialPrice : ite.goodsPrice) * parseInt(ite.goodsNum)
            ...
            if (ite.goodsNum) {
              ...
              ite.goodsStandardList.forEach((v) => {
                v.goodsStandarValList.forEach((vm) => {
                  if (vm.active) {
                    goodsAttribute.push(v.standardName + ': ' + vm.standardValName)
                    goodsAttributeIds.push(vm.id)
                    // obj = {
