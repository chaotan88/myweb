    ...
    methods: {
      ...
      */
      getSubclassDataList (columnId) {
        ...
        this.$http.post(this.api.getSetmealGoodsListByColumnId, {
          ...
          // 取属性的第一个值
          this.subclassDataList.forEach((item) => {
            let orderAttributeRequest = [] // 订单属性集合
            let arrtObj = {}
            if (item.goodsStandardList && item.goodsStandardList.length > 0) {
              let attributeName = []
              item.goodsStandardList.forEach((its) => {
                its.goodsStandarValList.forEach((ite) => {
                  attributeName.push(its.standardName + ': ' + ite.standardValName)
                  if (ite.attributeId) {
                    if (ite.attributeValList && ite.attributeValList.length > 0) {
                      arrtObj = {
                        attributeId: ite.attributeId,
                        attributeValId: ite.attributeValList[0].attributeValId || ''
                      }
                      orderAttributeRequest.push(arrtObj)
                    }
                  }
                  this.$set(item, 'goodsAttribute', attributeName.toString())
                  this.$set(item, 'orderAttributeRequest', orderAttributeRequest)
      ...
      */
      goPay () {
        ...
        this.subclassDataList.forEach((item) => {
          let attributeIds = []
          let goodsAttributeValIds = []
          item.goodsAttributeList.forEach((ite) => {
            attributeIds.push(ite.attributeId ? ite.attributeId : '')
            goodsAttributeValIds.push(ite.attributeValList[0].goodsAttributeId ? ite.attributeValList[0].goodsAttributeId : '')
          })
          this.$set(item, 'goodsId', item.id)
          this.$set(item, 'attributeIds', attributeIds.toString())
          this.$set(item, 'goodsAttributeValIds', goodsAttributeValIds.toString())
