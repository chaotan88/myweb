<template>
  <div class="indexContainer" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="10" infinite-scroll-immediate-check="false">
    <div class="activityBg" :style="{backgroundImage: 'url(' + act12Bg + ')', backgroundSize:'100% 100%'}">
      <!-- 顶部背景 -->
      <div class="h-bg">
        <!--serch-->
        <div>
      ...
      getBaseInfo () {
        this.$http.post(this.api.getBaseInfoList, {}).then((res) => {
          ...
          resData.forEach((item) => {
            // console.log(item)
            if (item.type === 0) {
              this.act12Bg = item.bannerUrl
              this.webLogo = item.webLogo
            }
          })
        })
      }
...
<style  lang=less>
  .indexContainer{
    ...
    .h-bg{
      ...
      /*background: url('../../assets/images/my/index-ban-bg1.png') no-repeat;*/
    ...
    .nav{
      ...
      width: 90%;
      height: 3.5rem;
      /*background: url("../../assets/images/homeNew/column_bg(1).png") no-repeat 100% 100%;*/
      margin: 0 auto;
      position: relative;
  ...
  .indexContainer .nav .vux-indicator .vux-icon-dot.active{
    background: #ffbf87;
  }
</style>
