    ...
    <div class="sharePurchase-wrap">
      <div v-if="asd">
        ...
        <div v-if="ggShow">
          ...
          <!--<div class="share-bg" v-else></div>-->
        </div>
        <!-- 素材背景 -->
        <div v-if="scShow">
          <div class="share-bg" v-if="bgControl">
            <img :src="sucaiList" v-if="sucaiList">
          </div>
          <div class="share-bg" v-else>
          </div>
        </div>
        <!-- 运营助手 -->
        <operate-assistant v-if="scShow"></operate-assistant>
      </div>
      <template v-if="!noDataSte && remoedList.length > 0">
        ...
        <div class="list-type-one">
          <div class="list-title">
            <img src="../../assets/images/action/sharePurchase-list-t-l.png">
            <span>推荐商品</span>
            <img src="../../assets/images/action/sharePurchase-list-t-r.png">
          </div>
          <ul>
            <li v-for="(item, index) in remoedList" @click="goDetail(item)">
              <div class="list-box">
                <div class="list-img">
                  ...
                  <div class="con-flex">
                    ...
                    <div class="con-price">
                      <template v-if="item.whetherShare">
                        <div><i>￥</i>{{item.goodsPrice | moneyFilter('', true)}}</div>
                        <div><i>￥</i>{{item.goodsSharePrice | moneyFilter('', true)}}</div>
                      </template>
                      <template v-else>
                        <div></div>
                        <div><i>￥</i>{{item.goodsPrice | moneyFilter}}</div>
                      </template>
                    </div>
                  </div>
...
<script>
  ...
  import operateAssistant from '../../components/operateAssistant'                        // 运营助手
  export default {
    components: {
      Swiper,
      SwiperItem,
    ...
    methods: {
      ...
      getShareMes () {
        this.$http.get(this.api.getShareMes, {}).then((res) => {
          ...
          this.$http.post(this.api.getSourceList, {id: this.shareConfigData.buySourceId}).then((res) => {
            // console.log(res)
            this.sucaiList = res.data.data.backgroundImg
            // 是否展示素材背景图
            if (res.data.data.backgroundOpen === 1) {
              this.bgControl = true
            } else {
              this.bgControl = false
            }
            // 关闭素材展示
            if (res.data.data.sourceStatus === 1) {
              this.asd = true
            } else {
              this.asd = false
            }
      ...
      toImage (v) {
        ...
        this.shareCon.alertTpye = 'purchase'
    ...
    .list-type-one{
      ...
      ul{
        li{
          ...
          -webkit-box-shadow: 0 0 10px 0 hsla(0,6%,58%,.3);
          .list-box{
            height: 3.22rem;
            display: flex;
            display: -webkit-flex;
            ...
            .list-con{
              ...
              .con-btn{
                ...
                right: .3rem;
                bottom: .2rem;
                background: linear-gradient(90deg, #fd8556, #f31f23);
                border-radius: 30px;
                color: #fff;
                font-size: .28rem;
              }
