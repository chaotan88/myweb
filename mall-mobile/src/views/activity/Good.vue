  ...
  <div class="shareDetail-wrap">
    ...
    <div class="goodDetail">
      <div class="baseInfo" id="baseInfo">   
        <!-- 列表切换 -->
        <div class="detail-share-list" v-if="remoedList.length > 0" id="detailShareList">
          <ul>
            <li v-for="(item, index) in remoedList" :class="item.active? 'active' : ''" @click="shareListTap($event, index, item)">
              <div>
        ...
        <div class="detail-share-list-hei"></div>
        <!-- 轮播图 -->
      ...
      <div class="footer-box">
        ...
        <div class="footer-box-btn" v-if="fromType == 2" @click="toImage(singShareDeta.goodsId)">
          <p>分享赚</p>
          <p>赚{{singShareDeta.oneGetCashPoints}}通用积分</p>
        </div>
      </div>
      <!-- 说明弹窗 -->
...
<script>
  ...
  import { Swiper, TransferDom, Popup, Icon, XDialog, SwiperItem, Qrcode } from 'vux'       // 轮播图,底部弹框
      ...
      toImage (id) {
        ...
        this.$http.get(this.api.immediatelyShare, {
          ...
          if (parseInt(resData.status) !== parseInt(this.api.ERR_OK) || !res) {
            ...
            if (resData.data.mainImage && resData.data.mainImage.match(/http/gi)) {
              this.getImageBase64ByUrl(resData.data.mainImage, (res) => {
            ...
            this.codeUrl = window.location.href.match(/.*mshop/gi)[0] + '/share/goodDetail?goodsId=' + id + '&shareInvitationCode=' + resData.data.invitationCode + '&fromType=1&shopId=' + shopId
            // 生成图片
            setTimeout(() => {
