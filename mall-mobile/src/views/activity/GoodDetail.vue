  ...
  <div class="shareDetail-wrap">
    ...
    <div class="goodDetail">
          ...
          <ul>
            ...
            <template v-if="fromType == 1">
              ...
              <li v-if="basicGoodsInfor.beInviterIfGetGoldPoints">
                <p>送</p>
                <p>{{basicGoodsInfor.beInviterGetGoldPoints}}芯豆</p>
              </li>
            </template>
            <!-- 分享进入的 -->
      ...
      <div class="footer-box">
        <div class="footer-box-img" @click="goSharePurchase">
      ...
      <div class="get-coin-pop" v-if="fromType == 1 && basicGoodsInfor.beInviterIfGetGoldPoints">
        <x-dialog v-model="showGetCoinPop" class="dialog-demo" :hide-on-blur="true" :dialog-style="{'max-width': '5.7rem', width: '100%', height: '50%', 'background-color': 'transparent'}">
          ...
          <p class="pop-num">获得{{basicGoodsInfor.beInviterGetGoldPoints}}芯豆</p> 
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      confirmOrder () {
        ...
        this.$http.post(`${this.api.getBuySoon}`, {
          ...
          if (res.data.status === this.api.ERR_OK) {
            ...
            localStorage.setItem('CONFIRM_ORDER_TYPE', 2)                                  // 下单的类型 1:普通购物，2：分享订单 3： 芯豆购买 4： 消费积分购买 5：联盟积分购买
            setTimeout(() => {
      ...
      */
      goSharePurchase () {
        this.$router.push({path: '/sharePurchase', query: {shareInvitationCode: this.$route.query.shareInvitationCode}})
        // localStorage.setItem('PAGE_INV_CODE', this.$route.query.shareInvitationCode)    // 存储地址栏邀请码
      }
    },
