<template>
  <div class="my">
    <!--用户信息-->
    <div class="user-box">
      <ul class="user-ul" @click="editPersonalData">
        <li class="img-li">
          <img src="../../assets/images/default-avatar.png" >
        </li>
        <li class="infor-li">
          <!-- <span>{{ personalData.name}}</span> -->
          <div v-if="personalData.user" class="personmsg-t">
      ...
      <img src="../../assets/images/my/seting.png" class="user-set" @click.stop="$router.push('/set')">
    </div>
    <!--订单、余额-->
      ...
      <div class="orderBalance-integral">
        <ul>
          <li @click="$router.push('/myBalance')">
            <div>{{totalIntegral.balance}}</div>
            <div>我的余额</div>
          </li>
          <li @click="$router.push('/myPoints')">
            <div>{{totalIntegral.bonusPoints}}</div>
            <div>消费积分</div>
          </li>
        </ul>
        <div class="delimiter">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      upgradeImmediately () {
        ...
        if (parseInt(this.personalData.user.cardStatus) === 1) {
          ...
          if (parseInt(this.registerStatus) === 0) { // 已认证未注册
            // 是否设置了地区
            if (!this.personalData.user.userProvinceCode || parseInt(this.personalData.bindStatus) === 0) {
              this.$router.push({path: '/region', query: {isSetName: 'true'}})
            } else {
      ...
      */
      goPointsSys () {
        window.location.href = window.location.href.split('mshop')[0] + 'points'
      }
    }
  }
</script>
