    ...
    <template slot="main">
      ...
      <table class="admin-gray-table">
        ...
        <tr>
          ...
          <td>联盟积分：</td>
          <td>{{detailsData.leaguePoints | filterMoney | filterEmpty('元')}}</td>
        </tr>
        <tr>
          <td>芯豆：</td>
          <td colspan="3">{{detailsData.goldPoints | filterMoney | filterEmpty('个')}}</td>
        </tr>
      </table>
      ...
      <template v-if="rechargeDetails.rechargeType === 2">
        ...
        <table class="admin-gray-table">
          <template v-if="rechargeDetails">
            ...
            <tr>
              <td>充值类型：</td>
              <td>{{rechargeDetails.rechargeSource | filterRechargeType | filterEmpty()}}</td>
              <td>是否自动分佣：</td>
              <td>{{rechargeDetails.commissionFlag | filterMaid | filterEmpty()}}</td>
            </tr>
            ...
            <tr>
              <td>充值时间：</td>
              <td colspan="3">{{rechargeDetails.addTime | filterDate | filterDate}}</td>
            </tr>
          </template>
          <tr v-else>
        ...
        <table class="admin-gray-table">
          <template v-if="rechargeDetails">
            ...
            <tr>
              ...
              <td>扣款时间：</td>
              <td>{{rechargeDetails.addTime | filterDate | filterEmpty}}</td>
            </tr>
          </template>
          <tr v-else>
            <td><no-data style="padding: 50px;"></no-data></td>
      ...
      <table class="admin-gray-table">
        ...
        <tr>
          <td>审核时间：</td>
          <td :colspan="parseInt(rechargeDetails.auditStatus) === 3 ? 0 : 3">{{rechargeDetails.auditTime  | filterDate | filterEmpty}}</td>
