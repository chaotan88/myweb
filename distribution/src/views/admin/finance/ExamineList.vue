  ...
  <common-tpl class="finance-index-wrap">
    ...
    <template slot="main">
      <el-table :data="tableData" style="width: 100%" v-loading="loading" element-loading-text="加载中">
        ...
        <el-table-column prop="consumePoints" label="消费积分（个）">
        ...
        <el-table-column prop="consumePoints" label="通用积分（个）">
          <template slot-scope="scope">
            <template v-if="scope.row.cashPoints">{{scope.row.rechargeType === 1 ? '-' : '+'}}{{scope.row.cashPoints | filterEmpty}}</template>
        ...
        <el-table-column prop="addTime" label="申请时间" min-width="120">
    ...
    <template slot="other">
      ...
      <examine-dialog :visible="examineVisible" :pageType="2" :actionType="actionType" :initData="examineData" @close="examineVisible = false" @success="handleSuccess">
      </examine-dialog>
