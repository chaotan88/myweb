    ...
    <template slot="main">
      <div class="goods-common-details">
        ...
        <table>
          ...
          <tr>
            <td>收件人地址：</td>
            <td v-if="detailsData.orderStatus === 2 && pageType !== 1" style="display: flex;">
            ...
            <td v-else>{{(detailsData.province + detailsData.city + detailsData.zone + detailsData.address) | filterEmpty}}</td>
          </tr>
          <tr v-if="pageType === 1">
            <td>发货时间：</td>
            <td >{{detailsData.sendGoodsTime | filterDate | filterEmpty}}</td>
          </tr>
          <tr  v-if="pageType === 1">
            <td>快递类型：</td>
            <td>{{detailsData.logisticsName | filterEmpty}}</td>
          </tr>
          <tr  v-if="pageType === 1">
            <td>物流单号：</td>
            <td >{{detailsData.logisticsNumber | filterEmpty}}</td>
          </tr>
        </table>
        ...
        <div class="ta-l view-authinfo-wrap">
          <template>
            <el-form :model="addForm" :rules="rules" ref="addForm" label-position="right" label-width='120px'>
              ...
              <el-form-item inline label='快递类型：' prop='logisticsNo' class="address-wrap">
              ...
              <el-form-item inline label='快递单号：' prop='logisticsNumber'>
                <el-input style="width: 200px;" placeholder='请输入物流单号,限50个字符' v-model="addForm.logisticsNumber"></el-input>
              </el-form-item>
              <div class="ta-c btn-wrap">
                <el-button type="primary" class='confirm-btn' :loading="confirmLoading" @click="submitForm('addForm')">确定</el-button>
              </div>
            </el-form>
          </template>
        </div>
