    ...
    <template slot="main">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="登录名" prop="name">
          ...
          <el-input v-model="ruleForm.name" placeholder="限20个字符"></el-input>
        </el-form-item>
        <el-form-item label="输入密码" prop="password" :required="!isEditPage">
          <el-input type="password" v-model="ruleForm.password" placeholder="限20个字符"></el-input>
        </el-form-item>
        <el-form-item label="确认密码" prop="newPassword" :required="!isEditPage">
          <el-input type="password" v-model="ruleForm.newPassword" placeholder="限20个字符"></el-input>
        </el-form-item>
...
export default {
  data () {
    var validateName = (rule, value, callback) => {
      ...
      if (value && value.length > 20) return callback(new Error('限20个字符'))
      callback()
    }
    var validatePassword = (rule, value, callback) => {
      ...
      if (value && (value.length > 20 || value.length < 6)) return callback(new Error('限6~20个字符'))
      callback()
    }
    var validateNewPassword = (rule, value, callback) => {
      ...
      if (value && (value.length > 20 || value.length < 6)) return callback(new Error('6~限20个字符'))
