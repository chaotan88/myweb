<!-- 账户信息 -->
<template>
  <common-tpl class="my-info-wrap" footer>
    ...
    <template slot="main">
      ...
      <el-form label-position="right" :model="formData" :rules="rules" ref="form" label-width="240px" class="basic-form-wrap">
        <el-form-item label="登录名：">
          <span class="phone">{{userAllInfo.userName | filterEmpty}}</span>
        </el-form-item>
        <el-form-item label="权限组 ：">
          <span class="phone">{{userAllInfo.groupName | filterEmpty}}</span>
        </el-form-item>
        <el-form-item label="手机号：">
          <span class="phone">{{userInfo.phone | filterPhone}}</span>
          <el-button @click="showDialogHandle('手机号验证', 'showCheckPhone')">{{!userInfo.phone ? '设置' : '修改'}}</el-button>
        </el-form-item>
...
import AuthEnterprise from './AuthEnterprise'
