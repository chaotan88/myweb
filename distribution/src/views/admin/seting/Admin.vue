  ...
  <common-tpl class="seting-admin-wrap">
    ...
    <template slot="main">
      ...
      <div class="ta-c common-bottom-wrap handleup-b">
        <el-pagination background layout="prev, pager, next" :current-page="currentPage" v-if="totalPages" :total="totalPages" @current-change="pageChange"></el-pagination>
      </div>
    </template>
  ...
  methods: {
    ...
    getListData () {
      ...
      this.$http.post('@ROOT_API/admin/list', {
        ...
        this.totalPages = resData.data.totalPages * this.pageSize        // 自定义好的列表总条数为0，需要传值给后台数据，循环后台传回来的数据
