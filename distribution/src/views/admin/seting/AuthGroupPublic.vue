      ...
      <el-form :model="formData" :rules="rules" ref="auth-form" label-width="240px">
        <el-row class="row-wrap">
          <el-form-item label="权限组名称：" prop="name" class="item-name">
            <el-input v-model="formData.name" maxlength="50" placeholder="限50个字符"></el-input>
          </el-form-item>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    */
    getAdminInfo () {
      this.$http.post('@ROOT_API/manageAuthController/info', {
        id: this.authId
      }).then((res) => {
    ...
    */
    getGroup () {
      this.$http.get('@ROOT_API/manageAuthController/menus').then((res) => {
        let resData = res.data
    ...
    */
    submitForm (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        ...
        this.loading = true
        let url = this.authId ? '@ROOT_API/manageAuthController/update' : '@ROOT_API/manageAuthController/add'
        this.$http.post(url, {
          id: this.authId,
