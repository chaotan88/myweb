      ...
      <div class="formTop-wrap">
        ...
        <el-row class="third-row">
          <el-form-item label="位置：" class='region-select-item'>
            ...
            <div><el-input v-model="formData.address" placeholder="请填写详细地址" class="detailedAddress" clearable></el-input></div>
            <!-- 百度地图 -->
            <div class="baidu-map">
              <baidu-map
                class="map"
        ...
        <el-row class="forth-row">
          <el-form-item label="经纬度：" class="latitude-longitude-wrap">
            <el-input v-model="formData.longitude" placeholder="请填写经度" class="detailedAddress" clearable></el-input>
            <el-input v-model="formData.latitude" placeholder="请填写纬度" class="detailedAddress" clearable></el-input>
          </el-form-item>
        </el-row>
      </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        dialogImageUrl: '',
        dialogVisible: false,
        validateGuardOfTerminalName: true,
        validateGuardOfAppScenario: true,
        validateGuardOfTerminalCoding: true,
        validateGuardOfAppOs: true,
        validateGuardOfTerminalType: true,
        validateGuardOfContact: true,
        validateGuardOfPhone: true
      }
    },
    computed: {
      center () {
    ...
    methods: {
      ...
      getTerminalDetail (id) {
        this.$http.post('@ROOT_API/terminalManage/info', {id: id})
          .then((res) => {
...
//                console.log('resData.data.provinceCode: ', resData.data.provinceCode, resData.data.province)
//                console.log('resData.data.provinceCode: ', resData.data.cityCode, resData.data.city)
//                console.log('resData.data.provinceCode: ', resData.data.zoneCode, resData.data.zone)
            this.regionData.province.code = resData.data.provinceCode || ''
            this.regionData.province.name = resData.data.province || ''
            this.regionData.city.code = resData.data.cityCode || ''
            this.regionData.city.name = resData.data.city || ''
            this.regionData.area.code = resData.data.zoneCode || ''
            this.regionData.area.name = resData.data.zone || ''
      ...
      handleFileList () {
        this.formData.adPositionAttachmentList.forEach((row) => {
          this.fileArr1.push({name: '', id: row.id, url: this.$dm.ROOT_IMG + row.url})
        })
      },
      ...
      handleSuccess (response, file, fileList) {
        ...
        this.formData.adPositionAttachmentList.push({id: '', url: response.data})
      },
      ...
      handleRemove (file, fileList) {
        this.formData.adPositionAttachmentList = []
        fileList.forEach((row) => {
          this.formData.adPositionAttachmentList.push({id: '', url: row.url})
        })
      },
      submitForm () {
        ...
        let rootUrl = this.$dm.ROOT_IMG
        this.formData.adPositionAttachmentList.filter((row) => {
          row.id = parseInt(row.id) || ''
          row.url = row.url.replace(rootUrl, '')
        })
        if (terminalNameStatus && appScenarioStatus && terminalCodingStatus && appOsStatus && terminalTypeStatus && contactStatus && phoneStatus) {
          this.$http.post(this.$dm.ROOT_API + 'terminalManage/modify?clientType=2&token=' + this.token, this.formData).then((res) => {
            let resData = res.data
            if (resData.status !== '1') {
              this.$message({
                message: resData.msg,
