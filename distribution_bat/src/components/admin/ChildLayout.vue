<template>
  <div class="admin-childlayout-wrap">
    <div class="pos-a slide-nav-wrap">
      ...
      <el-menu :collapse="isCollapse" :default-openeds="currentRouters" :unique-opened="true" :key="menuKey">
      ...
      if (row.path.match(/order/)) {
        row.children.forEach((list) => {
          if (list.path.match(/supply/)) {
            let userInfo = JSON.parse(localStorage.getItem(this.$global.USER_INFO))
            list.meta.visible = parseInt(userInfo.shopId) !== 1 ? !!false : !!true
          }
        })
      }
    })
