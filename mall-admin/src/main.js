...
import Utils from '@/global/function'  // 全局函数
...
$http.interceptors.request.use(config => {
  ...
  let params = (config.url.indexOf('?') === -1 ? '?' : '&') + 'clientCode=4&versionNumber=v1.0&companyNo=PP008&belongSystem=6&token=' + (userInfo.token ? userInfo.token : '')
