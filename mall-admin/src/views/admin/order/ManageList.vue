<!-- 订单管理 -->
<template>
  <common-tpl class="order-index-wrap" :footer="false">
        ...
        <div slot="main">
          <el-form :model="formData" :rules="rules" ref="formData" label-width="100px" class="demo-formData">
            <el-form-item label="订单编号">
              <el-input placeholder="请输入订单编号" v-model.trim="formData.orderNo"></el-input>
            </el-form-item>
            ...
            <el-form-item label="买家姓名">
              <el-input v-model.trim="formData.cardName" placeholder="请输入买家姓名"></el-input>
            </el-form-item>
            <el-form-item label="买家手机号" prop="customerPhone">
              <el-col :span="11">
                <el-input v-model.trim="formData.customerPhone" placeholder="请输入手机号"></el-input>
              </el-col>
            </el-form-item>
            <template v-if="!isFinanceExamine && pageType === ''">
              <el-form-item label="订单状态">
                <el-select v-model="formData.orderStatusCopy" placeholder="请选择订单状态">
              ...
              <!-- <el-form-item label="支付状态" class="full-w" v-if="pageType === 0">
              ...
              <el-form-item label="支付方式" class="full-w">
    ...
    <template slot="main">
      <el-table :data="tableData" style="width: 100%" v-loading="loading" element-loading-text="加载中">
        <el-table-column fixed prop="orderNo" label="订单编号" min-width="180"></el-table-column>
        <template v-if="!isFinanceExamine">
          <el-table-column prop="cardName" label="买家姓名" min-width="150">
            <template slot-scope="scope">
              <div class="truncate">{{scope.row.cardName | filterEmpty}}</div>
            </template>
          </el-table-column>
          <el-table-column prop="customerPhone" label="买家手机号" min-width="150">
            <template slot-scope="scope">{{scope.row.customerPhone | filterEmpty}}</template>
          </el-table-column>
          <el-table-column prop="" label="收货人姓名" min-width="100">
        ...
        <el-table-column prop="payStatus" label="支付状态" min-width="140">
          ...
          <template slot-scope="scope">
            <el-dropdown class="d-b">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="isFinanceExamine && pageType === 4">
                ...
                <template v-if="!isFinanceExamine">
                  <el-dropdown-item v-if="scope.row.orderStatus !== 0 && scope.row.orderStatus !== 2">
                ...
                <template v-else>
                  <el-dropdown-item v-if="pageType !== 4">
                    <div @click="$router.push({path: '/admin/finance/balance/details', query: {id: scope.row.orderId}})"><i class="icon el-icon-view"></i>详情</div>
                  </el-dropdown-item>
                </template>
                ...
                <el-dropdown-item v-if="scope.row.orderStatus === 3 && !isFinanceExamine">
                  <div @click="logisticsBeforeHandle(scope.row)"><i class="icon el-icon-location-outline"></i>查看物流</div>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
...
export default {
  ...
  data () {
    ...
    let validatePhone = (rule, value, cb) => {
      let reg = /^\d{11}$/gi
      if (value && !value.toString(reg).match()) return cb(new Error('手机格式错误'))
      cb()
    }
    return {
  ...
  mounted () {
    ...
    if (this.$route.path.match(/admin\/finance/gi)) {
      ...
      let matchPath = this.$route.path.match(/index|payment|delivery|receipt|evaluate|complete|cancel/gi)
      if (matchPath) {
        switch (matchPath[0]) {
          case 'index':
            this.pageType = ''
    ...
    if (this.formData.orderType === 2) {
      if (this.$Utils.filterSamePage(2) || this.$Utils.filterSameModule('/admin/order/supply')) {
      ...
      if (this.$Utils.filterSamePage(2) || this.$Utils.filterSameModule('/admin/order/self')) {
        if (getPagetype && parseInt(getPagetype) === this.pageType || getPagetype === this.pageType) {
          let res = JSON.parse(localStorage.getItem(this.$global.FORM_DATA))
          if (res) this.formData = res
        }
  ...
  methods: {
    ...
    getListData (type) {
      ...
      if (!this.isFinanceExamine) {
        ...
        orderStatus = ''
        url = '@ROOT_API/buySupplierSettleFormController/getSupplierWaitOrderList'
      }
      if (type) {
        if (!this.isFinanceExamine) {
          url = 'order/exportOrderList'
        } else {
          url = 'buySupplierSettleFormController/exportSupplierWaitOrderList'
        }
      } else {
        this.loading = true
      }
      let data = {
        ...
        orderStatus,
        // orderStatus: this.isFinanceExamine ? '' : this.pageType,
        ...
        payType: this.formData.payType,     // 是 string  支付方式 1 微信 2 支付宝 3 线下支付
        payStatus,      // 是 string  支付状态 1 未付款 2已付款 3已退款
        customerPhone: this.formData.customerPhone,   // 是 string  客户手机号
        customerName: this.formData.cardName,         // 是 string  客户名称
      ...
      if (!type) {
        ...
        // console.log('this.$global.PARAMS: ', this.$global.PARAMS)
