  ...
  <div class="pos-r relation-public-wrap">
    ...
    <div class="table-left fl-l">
      ...
      <div class="pos-r tabs-wrap">
        ...
        <el-select placeholder="请选择店铺" class="pos-a" v-model="leftShopId" v-if="leftTabs === 'shop'" @change="shopidChange">
      ...
      <el-table ref="multipleTable" :data="listLeftData" max-height="500" class="full-w" @selection-change="selectLeftHandle" v-elmore="handleLoadmore">
        ...
        <el-table-column type="mainImage" label="商品图片" width="80">
          <template slot-scope="scope">
            <template v-if="scope.row.mainImage">
              <a :href="scope.row.mainImage | filterImgUrl" target="_blank">
                <img style="width: 60px; height: 60px;" :src="scope.row.mainImage | filterImgUrl">
              </a>
            </template>
    ...
    <div class="table-right fl-r">
      ...
      <el-table ref="multipleTable" :data="listRightData" tooltip-effect="dark" max-height="500" class="full-w" @selection-change="selectRightHandle">
        <el-table-column type="selection" width="55"></el-table-column>
        ...
        <el-table-column prop="goodsNo" label="商品编码">
          <template slot-scope="scope">
            <span :class="{'red-color': scope.row.discount > compareDiscount}">{{scope.row.goodsNo | filterEmpty}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="goodsName" label="商品名称">
          <template slot-scope="scope">
            <span :class="{'red-color': scope.row.discount > compareDiscount}">{{scope.row.goodsName | filterEmpty}}</span>
          </template>
        </el-table-column>
        <!-- <el-table-column prop="goodsName" label="选择SKU">
          <template slot-scope="scope">
            <span class="cursor-p">请选择规格</span>
          </template>
        </el-table-column> -->
        <el-table-column prop="goodsName" label="设置数量" v-if="setmealType === 1">
          <template slot-scope="scope">
            <!-- <el-input-number type="number" v-model="scope.row.goodsNum" controls-position="right" :min="1" precision="1" @change="numberChange(scope.row)"></el-input-number> -->
            <el-input type="number" v-model.number="scope.row.goodsNum" @blur="numberChange(scope.row)" @change="numberChange(scope.row)"></el-input>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="选择SKU" :visible.sync="selectVisible">
      <el-form>
        <p>尺码</p>
        <el-radio-group>
          <el-radio :label="1">33cm</el-radio>
          <el-radio :label="1">34cm</el-radio>
          <el-radio :label="1">35cm</el-radio>
          <el-radio :label="1">36cm</el-radio>
        </el-radio-group>
        <p>颜色</p>
        <el-radio-group>
...
export default {
  ...
  mounted () {
    if (this.$route.path.match(/setmeal/gi)) this.moduleType = 2
  ...
  methods: {
    ...
    getRelationAllGoodsList (page) {
      ...
      this.loading = true
      let url = this.moduleType === 2 ? '@ROOT_API/column/allSetMealGoodsList' : '@ROOT_API/column/allGoodsList'
      this.$http.post(url, {
    ...
    */
    moveToRightHandle () {
      ...
      this.listLeftRes = []
      if (this.listLeftData.length < 10) this.getRelationAllGoodsList()
      this.$emit('change', this.listRightInitData, this.deleteListData)
    },
    ...
    numberChange (row) {
      ...
      this.listRightData.forEach((list) => {
        if (row.id === list.id) {
