...
<template>
  <common-tpl class="add-group-wrap" footer back>
        ...
        <el-form-item prop="selectList" class="table-item-wrap">
          <table cellpadding="0" cellspacing="0" class="ta-l table-wrap">
            ...
            <tr v-for="item in groupData" :key="item.id">
              ...
              <td>
                <el-checkbox v-for="list in item.childMenuList" v-model="item.selectList" :label="list.id" :value="list.id" :key="list.id" :disabled="item.disabled === true" @change="listSelectChange(item)">{{list.meunName}}</el-checkbox>
              </td>
            </tr>
          </table>
    ...
    <template slot="footer">
      <el-button type="primary" :loading="loading" @click="submitForm('auth-form')">确定</el-button>
    </template>
