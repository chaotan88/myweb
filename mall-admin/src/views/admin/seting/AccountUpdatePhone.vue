  ...
  <div class="ta-l update-phone-wrap">
    ...
    <template v-else>
      <el-form :model="verifiData" :rules="rules" ref="verifiPhone" label-position="right" label-width='100px'>
        ...
        <div class="ta-c next-btn-wrap">
          <el-button size="medium" type="primary" :loading="confirmLoading" @click="submitFormBefore('verifiPhone')">确定</el-button>
        </div>
      </el-form>
    </template>
  </div>
...
export default {
  ...
  data () {
    ...
    let validatePhone = (rule, value, callback) => {
      if (!this.phone && !value) return callback(new Error('请输入您的手机号码'))
    ...
    return {
      ...
      confirmLoading: false,  // 下一步loading
  ...
  methods: {
    ...
    submitForm () {
      this.confirmLoading = true
      this.$http.post('@ROOT_API/supplier/editSupplierContactPhone', {
        id: this.initData.id,
        userId: this.initData.userId,
        contactPhone: this.verifiData.newPhone
      }).then((res) => {
        let resData = res.data
        if (parseInt(resData.status) !== 1) {
          this.$message({
