    ...
    <template slot="main">
      <el-table :data="tableData" style="width: 100%" v-loading="loading" element-loading-text="加载中">
        ...
        <el-table-column fixed="right" label="操作" width="200">
          <template slot-scope="scope">
            <template v-if="scope.row.roleType === 1 || scope.row.loginName === userInfo.userName">超级管理员不可编辑</template>
            <template v-else>
  ...
  methods: {
    ...
    getListData () {
      ...
      this.$http.post('@ROOT_API/admin/list', {
        ...
        this.pageData.total = results.totalPages * this.pageData.pageSize        // 自定义好的列表总条数为0，需要传值给后台数据，循环后台传回来的数据
        this.tableData = results.dataList
      }).finally(() => {
        this.loading = false
      })
    },
