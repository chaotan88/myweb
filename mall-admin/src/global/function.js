...
module.exports = {
   ...
   * 判断路径及参数是否绝对相等
   */
  filterAbsoltePath (number = 3) {
    if (!this.filterSamePage(number)) return false
    let historyRoute = JSON.parse(localStorage.getItem(global.HISTORY_ROUTE)) || []
    let res = false
    if (parseInt(number) === 2 && historyRoute[historyRoute.length - 1].query === historyRoute[historyRoute.length - 2].query) {
      res = true
    } else if (parseInt(number) === 3 && historyRoute[0].query === historyRoute[1].query && historyRoute[1].query === historyRoute[2].query) {
      res = true
    }
    return res
  },
